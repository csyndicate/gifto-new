...
$result = $wpdb->get_results(
    'SELECT transaction_time,transaction_id,product_name,price,currency,receivedmail,brandname,amount FROM transaction_status where mail_status = "pending"'
);
...
foreach ($result as $key => $tranid) {
	echo $time = $tranid->transaction_time;
    ...
    echo $brandnme = $tranid->brandname;
    echo $amtvouher = $tranid->amount;
    //echo  $ctreatedtime = $tranid->tran_time;
    ...
    if ($data) {
        ...
        foreach ($result_data2 as $new1) {
           ...
           // $c_date = date("Y-m-d H:i:s", $t);
            $rt = time();
                $r_date = date("Y-m-d H:i:s", $rt);
            $sql = $wpdb->insert("vouchercards", [
                "created_at" => $r_date,
                "transaction_id" => $ids,
                "api_name" => $prov,
                "redeem_pin" => $newpin,
                "redeem_code" => $newcod,
                "order_status" => $statusv,
                "amountv" => $amtvouher,
                "brandv" => $brandnme,
            ]);
...
';
            ...
            $wpdb->query(
                $wpdb->prepare(
                    "UPDATE transaction_status SET mail_status='done' WHERE transaction_id=$ids"
                )
            );
        } elseif ($newcod) {
            $to = $mail3; //sendto@example.com
