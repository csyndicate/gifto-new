...
function minimog_notice_required_plugin_version( $plugin_name, $plugin_version, $show_link = false ) {
	...
	$message = sprintf( esc_html__( '%1$s requires %2$s version %3$s or greater.', 'minimog' ), '<strong>' . MINIMOG_THEME_NAME . '</strong>', '<strong>' . $plugin_name . '</strong>', $plugin_version );
...
/**
 * Allow to remove method for an hook when, it's a class method used and class don't have variable, but you know the
 * class name :)
 *
...
function minimog_get_wc_template_part( $slug, $name = '', $args = array() ) {
	...
	if ( ! $template ) {
		if ( $name ) {
			$template = WC_TEMPLATE_DEBUG_MODE ? '' : locate_template( array(
				"{$slug}-{$name}.php",
