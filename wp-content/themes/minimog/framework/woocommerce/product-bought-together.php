...
class Product_Bought_Together {
	...
	public function show_items( $product = null, $is_custom_position = false ) {
		...
		foreach ( $items as $key => $item ) {
			...
			if ( ! empty( $_item['product'] ) && $_item['product'] instanceof \WC_Product && ( ! in_array( $_item['product']->get_type(), self::$types, true ) || ( ( self::get_setting( 'exclude_unpurchasable', 'no' ) === 'yes' ) && ( ! $_item['product']->is_purchasable() || ! $_item['product']->is_in_stock() ) ) ) ) {
				unset( $items[ $key ] );
				continue;
			}
