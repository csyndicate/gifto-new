	...
	class Minimog_Woo {
		...
		public function initialize() {
			...
			if ( class_exists( 'WooCommerce_simple_auction' ) ) {
				minimog_require_file_once( MINIMOG_WOO_CORE_DIR . '/3rd-party-compatible/auction.php' );
			}

			if ( class_exists( 'LumiseWoo' ) ) {
			...
			if ( defined( 'WCPAY_PLUGIN_FILE' ) ) {
				minimog_require_file_once( MINIMOG_WOO_CORE_DIR . '/3rd-party-compatible/woocommerce-payments.php' );
			}
			...
			if ( class_exists( 'WC_Subscriptions' ) ) {
				minimog_require_file_once( MINIMOG_WOO_CORE_DIR . '/3rd-party-compatible/woocommerce-subscriptions.php' );
			}
			...
			// Disable update.
			add_filter( 'wpc_check_update', '__return_false', PHP_INT_MAX );
			add_filter( 'wpc_dismiss_notices', '__return_true', PHP_INT_MAX );

			add_filter( 'body_class', [ $this, 'body_classes' ] );

			add_filter( 'woocommerce_get_image_size_gallery_thumbnail', [
				$this,
				'change_gallery_thumbnail_crop',
			], 999 );
		}

		/**
		 * Update product gallery thumbnail size.
		 *
		 * @param $size
		 *
		 * @return mixed
		 */
		public function change_gallery_thumbnail_crop( $size ) {
			$size['crop']   = 0;
			$size['height'] = $this->get_product_image_height_by_width( $size['width'] );
		...
		public function modify_theme_support() {
			add_theme_support( 'woocommerce', [
				'gallery_thumbnail_image_width' => 150, // Default is 100
			] );
		}
		...
		public static function get_remove_active_filter_links( $filters ) {
			...
			if ( isset( $filters['min_price'] ) || isset( $filters['max_price'] ) ) {
				$clear_link = $filter_link;
				$clear_link = remove_query_arg( [ 'min_price', 'max_price' ], $clear_link );

				if ( isset( $filters['min_price'] ) && isset( $filters['max_price'] ) ) {
					$clear_link_text = wc_price( $filters['min_price'] ) . ' - ' . wc_price( $filters['max_price'] );
				} elseif ( isset( $filters['min_price'] ) ) {
					$clear_link_text = sprintf( __( 'From %s', 'minimog' ), wc_price( $filters['min_price'] ) );
				} else {
					$clear_link_text = sprintf( __( 'To %s', 'minimog' ), wc_price( $filters['max_price'] ) );
				}
		...
		public function get_loop_product_image_size( $width = 0 ) {
			if ( ! empty( $width ) ) {
				$_width = intval( get_option( 'woocommerce_thumbnail_image_width', 450 ) );

				if ( $width !== $_width ) { // If it's custom size.
					if ( ! isset( self::$product_loop_image_size[ $width ] ) ) {
						self::$product_loop_image_size[ $width ] = $this->get_product_image_size_by_width( $width );
					}
		...
		public function get_single_product_image_size( $width = 0 ) {
			if ( ! empty( $width ) ) {
				$_width = intval( get_option( 'woocommerce_single_image_width', 600 ) );
		...
		public function get_single_product_image_size_by_feature_style( $feature_style, $is_quick_view = false ) {
			switch ( $feature_style ) {
				...
				case 'grid':
					...
					break;
				default: // Slider.
					$image_width = $is_quick_view ? 0 : Minimog_Woo::instance()->get_single_product_image_width();
					break;
			}
		...
		public function get_shop_loop_style_options() {
			return array(
				...
				'grid-11' => sprintf( esc_html__( 'Style %s', 'minimog' ), '11' ),
				'grid-12' => sprintf( esc_html__( 'Style %s', 'minimog' ), '12' ),
			);
		}
		...
		public function get_shop_loop_carousel_style_options() {
			return array(
				...
				'carousel-11' => sprintf( esc_html__( 'Style %s', 'minimog' ), '11' ),
				'carousel-12' => sprintf( esc_html__( 'Style %s', 'minimog' ), '12' ),
			);
		}
