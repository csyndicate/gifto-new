	...
	class Minimog_Title_Bar {
		...
		public function render_title() {
			...
			if ( Minimog_Woo::instance()->is_shop() ) {
				$heading_text = pll__( Minimog::setting( 'product_archive_title_bar_title' ) );
				...
				$heading_text = pll__( Minimog::setting( 'title_bar_home_title' ) ) . single_tag_title( '', false );
			} elseif ( is_tag() ) {
				$heading_text = pll__( Minimog::setting( 'title_bar_archive_tag_title' ) ) . single_tag_title( '', false );
			} elseif ( is_author() ) {
				$heading_text = pll__( Minimog::setting( 'title_bar_archive_author_title' ) ) . '<span class="vcard">' . get_the_author() . '</span>';
			} elseif ( is_year() ) {
				$heading_text = pll__( Minimog::setting( 'title_bar_archive_year_title' ) ) . get_the_date( esc_html_x( 'Y', 'yearly archives date format', 'minimog' ) );
			} elseif ( is_month() ) {
				$heading_text = pll__( Minimog::setting( 'title_bar_archive_month_title' ) ) . get_the_date( esc_html_x( 'F Y', 'monthly archives date format', 'minimog' ) );
			} elseif ( is_day() ) {
				$heading_text = pll__( Minimog::setting( 'title_bar_archive_day_title' ) ) . get_the_date( esc_html_x( 'F j, Y', 'daily archives date format', 'minimog' ) );
			} elseif ( is_search() ) {
				$heading_text = pll__( Minimog::setting( 'title_bar_search_title' ) ) . '"' . get_search_query() . '"';
			} elseif ( is_category() || is_tax() ) {
				$heading_text = pll__( Minimog::setting( 'title_bar_archive_category_title' ) ) . single_cat_title( '', false );
			} elseif ( is_singular() ) {
				...
				if ( '' === $heading_text ) {
					...
					switch ( $post_type ) {
						case 'post' :
							$heading_text = pll__( Minimog::setting( 'title_bar_single_blog_title' ) );
							break;
						case 'product' :
							$heading_text = pll__( Minimog::setting( 'product_single_title_bar_title' ) );
							break;
					}
				}
		...
		public function render_breadcrumb() {
			$breadcrumb_enable = Minimog::setting( 'breadcrumb_visibility' );
			$breadcrumb_enable = apply_filters( 'minimog/breadcrumb/visibility', $breadcrumb_enable );

			if ( '1' === $breadcrumb_enable ) {
				minimog_load_template( 'breadcrumb' );
			}
		}
	}
