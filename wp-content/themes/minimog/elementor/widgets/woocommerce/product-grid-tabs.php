...
class Widget_Product_Tabs extends Base {
	...
	public function get_name() {
		return 'tm-product-tabs';
	}

	public function get_title() {
		return esc_html__( 'Product Grid Tabs', 'minimog' );
	}
	...
	public function get_keywords() {
		return [ 'product', 'tabs', 'product tabs', 'product grid tabs' ];
	}
	...
	protected function add_product_tabs_section() {
		...
		$this->add_control( 'tabs_nav_style', [
			...
			'options' => [
				...
				'03'       => sprintf( esc_html__( 'Preset %s', 'minimog' ), '03' ),
				'04'       => sprintf( esc_html__( 'Preset %s', 'minimog' ), '04' ),
			],
			'default' => '',
		] );
		...
		$this->add_control( 'tabs', [
			...
			'title_field' => "{{{ title || MinimogElementor.helpers.getRepeaterTextForProductTabs('tm-product-tabs', 'tabs', 'query_source', query_source) }}}",
			'default'     => [
				[
					'title'        => '',
					'query_source' => 'latest',
						...
						<div class="minimog-tabs__header" role="tablist">
							...
							<?php foreach ( $product_tabs as $key => $product_tab ) : ?>
								...
								<div <?php $this->print_render_attribute_string( $tab_key ); ?>>
									...
									<span class="tab-title__text"><?php echo '' . $this->get_tab_title( $product_tab ); ?></span>
								</div>
							<?php endforeach; ?>
						</div>
					<?php endif; ?>
			...
			<div class="minimog-tabs__content">
				...
				<?php foreach ( $product_tabs as $key => $product_tab ) : ?>
					...
					if ( 1 === $loop_count ) {
						...
						$products = \Minimog\Woo\Ajax_Handlers::instance()->get_product_tab_content( $query_args );
					} else {
					...
					$this->add_render_attribute( $grid_key, 'class', str_replace( 'grid-', 'group-style-', $style ) );
					$this->add_render_attribute( $grid_key, 'class', 'tm-tab-product-element minimog-grid-wrapper minimog-product style-' . $style );
					$this->add_render_attribute( $grid_key, 'class', 'caption-style-' . $caption_style );
					$this->add_render_attribute( $grid_key, 'data-grid', wp_json_encode( $grid_options ) );
					?>
					<div <?php $this->print_render_attribute_string( $tab_key ); ?><?php echo 1 === $loop_count ? '' : ' hidden'; ?>>
						<div class="tab-content-wrapper">
							<div <?php $this->print_render_attribute_string( $grid_key ); ?>>
								<div class="minimog-grid lazy-grid">
									<div class="grid-sizer"></div>
									<?php if ( 1 === $loop_count ) : ?>
										<?php if ( $products['found'] ) : ?>
											<?php echo '' . $products['template']; ?>
										<?php endif; ?>
									<?php else: ?>
										<?php
										...
										<?php for ( $i = $number_posts; $i > 0; $i-- ): ?>
											<div class="grid-item grid-skeleton-item"
											     style="<?php echo '--skeleton-image-height: ' . $skeleton_image_height; ?>">
												<?php \Minimog_Woo::instance()->output_skeleton_loading_item( $loop_settings ); ?>
											</div>
										<?php endfor; ?>
									<?php endif; ?>
								</div>
								<div class="minimog-grid-response-messages"></div>
							</div>
						</div>
					</div>
				<?php endforeach; ?>
