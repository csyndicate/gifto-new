...
class Modify_Widget_Form extends Modify_Base {
	...
	public function section_form_fields( $element ) {
		...
		if ( ! empty( $fields['width']['options'] ) ) {
			// Append new options.
			$new_options = $fields['width']['options'] + [ 'fit' => esc_html__( 'Auto Fit', 'minimog' ) ];
	...
	public function section_buttons( $element ) {
		...
		if ( ! empty( $button_width['options'] ) ) {
			$button_width_options = $button_width['options'] + [ 'custom' => esc_html__( 'Custom', 'minimog' ) ];
		...
		$element->add_control( 'button_css_class', [
