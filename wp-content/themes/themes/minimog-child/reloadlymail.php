...
<?php
    ...
    $result = $wpdb->get_results('SELECT transaction_id,product_name,price,currency,receivedmail,brandname,amount FROM transaction_status where mail_status = "pending"');
...
foreach ($result_data2 as $new1){
	...
	$c_date= date('Y-m-d H:i:s',$t);		
					$sql = $wpdb->insert("vouchercards", array(
	"created_at" =>$c_date,				
   "transaction_id" => $ids,
    "api_name" => $prov,
	"redeem_pin" => $newpin,
