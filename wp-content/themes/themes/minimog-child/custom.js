...
//product page mobile validation
function phonenumber(contact){
		
		var phoneno = /^(\+|\d)[0-9]{6,10}$/;
...
//remove algolia from faq page
jQuery(document).ready(function(){
	jQuery('#epkb_search_terms').keyup(function(){
		//alert('test');
jQuery('.aa-dataset').hide();
});
});
//wallet functionality
function checkWallet() {
  let elm = document.querySelectorAll(".cart-totals-row.cart-totals-fee");
  elm = elm[elm.length - 1];
  let label = elm.querySelector(".cart-totals-label");
  let labeltext = elm.querySelector(".cart-totals-value");
  if (label && label.innerHTML.trim() === "") {
    labeltext.classList.add("test-wallets");
  }
//   console.log(itr)
//   itr++;
}
