...
class Enqueue {
	...
	public function initialize() {
		...
		add_filter( 'woocommerce_get_asset_url', [ $this, 'update_wc_scripts_src' ], 9999, 2 );
	...
	public function frontend_scripts() {
		...
		/**
		 * Since WC 7.8.0 wc-cart-fragments handle removed from global enqueue.
		 */
		$fly_cart_depends = [
			'jquery',
			'wc-cart-fragments',
		...
		if ( '1' === \Minimog::setting( 'shop_quick_view_enable' ) ) {
			...
			if ( '1' === \Minimog::setting( 'shop_quick_view_light_gallery_enable' ) ) {
	 ...
	 * Used edition version for better performance.
	 * Replace src instead dequeue to avoid missing scripts depends on it.
	 * Priority 99 to run after plugins.
	 *
	 * @version 2.8.0 Update src via wc asset filter instead of update src $wp_scripts->queue to avoid src can't update
	 *          on some ways.
	 */
	public function update_wc_scripts_src( $path, $file ) {
		$min = \Minimog_Enqueue::instance()->get_min_suffix();

		switch ( $file ) :
			case "assets/js/frontend/add-to-cart-variation{$min}.js": // wc-add-to-cart-variation.
				return MINIMOG_THEME_URI . "/assets/js/woo/add-to-cart-variation{$min}.js";
				break;
			case "assets/js/frontend/cart-fragments{$min}.js": // wc-cart-fragments.
				return MINIMOG_THEME_URI . "/assets/js/woo/cart-fragments{$min}.js";
				break;
			case "assets/js/frontend/single-product{$min}.js": // wc-single-product.
				return MINIMOG_THEME_URI . "/assets/js/woo/single-product{$min}.js";
				break;
		endswitch;
