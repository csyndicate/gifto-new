...
class Product_Attribute_Swatches {
	...
	const RECOMMEND_PLUGIN_VERSION = '1.7.0';
	...
	public function initialize() {
		...
		add_filter( 'woocommerce_get_sections_products', [ $this, 'wc_get_sections_products' ], 10, 2 );
		add_filter( 'woocommerce_get_settings_products', [ $this, 'wc_get_settings_products' ], 10, 2 );
	 ...
	 *
	 */
	public function wc_get_sections_products( $sections ) {
		$sections['minimog_variation_swatches'] = __( 'Variation swatches', 'minimog' );
	 ...
	 *
	 */
	public function wc_get_settings_products( array $settings, string $section_id ): array {
		if ( 'minimog_variation_swatches' !== $section_id ) {
			return $settings;
		}

		$title_item = array(
			'title' => __( 'Variation swatches', 'minimog' ),
			'type'  => 'title',
		);
		...
		$settings[] = array(
			'name'    => esc_html__( 'Variation out of stock style', 'minimog' ),
			'id'      => 'minimog_variation_swatches_out_of_stock_style',
			'type'    => 'radio',
			'options' => [
				''      => 'Normal',
				'cross' => 'Blur with cross',
				'hide'  => 'Hide',
			],
			'default' => '',
		);
	...
	public function body_classes( $classes ) {
		$variation_out_of_style_style = get_option( 'minimog_variation_swatches_out_of_stock_style' );

		if ( ! empty( $variation_out_of_style_style ) ) {
			$classes[] = 'variation-attribute-out-of-stock--' . $variation_out_of_style_style;
		}
