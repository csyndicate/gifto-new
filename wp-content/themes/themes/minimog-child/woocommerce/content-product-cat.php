...
$default_settings = [
	'style'           => Minimog_Woo::instance()->get_shop_categories_carousel_style(),
...
<div <?php wc_product_cat_class( $item_class, $category ); ?>>
	<div class="cat-wrap minimog-box">
		...
		<?php
		if ( in_array( $settings['style'], [ '03', '04' ], true ) ) {
			minimog_get_wc_template_part( 'cat-loop/caption', '03', [
				'category' => $category,
				'link'     => $link,
				'settings' => $settings,
			] );
		} else {
			minimog_get_wc_template_part( 'cat-loop/caption', $settings['style'], [
				'category' => $category,
				'link'     => $link,
				'settings' => $settings,
			] );
