	...
	<ul class="<?php echo esc_attr( $list_class ); ?>">
		...
		foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
			...
			if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_widget_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
				...
				$thumbnail         = Minimog_Woo::instance()->get_product_image( $_product, Minimog_Woo::instance()->get_loop_product_image_size( 108 ) );
				...
				<li class="woocommerce-mini-cart-item <?php echo esc_attr( apply_filters( 'woocommerce_mini_cart_item_class', 'mini_cart_item', $cart_item, $cart_item_key ) ); ?>">
					<div class="product-thumbnail">
						<?php if ( empty( $product_permalink ) ) : ?>
							<?php Minimog_Helper::e( $thumbnail ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
						<?php else : ?>
					...
					<div class="product-info">
						<h3 class="product-name post-title-2-rows">
							<?php if ( empty( $product_permalink ) ) : ?>
								<?php Minimog_Helper::e( $product_name ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
							<?php else : ?>
								<a href="<?php echo esc_url( $product_permalink ); ?>">
									<?php Minimog_Helper::e( $product_name ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
								</a>
