...
<table class="wt_smcpn_freevs_pro">
	...
	{
		?>
		<tr class="wt_sc_freevspro_table_hd_tr" data-index="<?php echo $index_i;?>" data-state='visible'>
			<td colspan="3"><span class="wt_sc_freevspro_table_hd_tr_dashicon<?php echo $index_i;?> dashicons dashicons-arrow-up-alt2"></span>&ensp;
				<?php
					if(!is_array($val_arr[0])){
						echo $val_arr[0];
					}else{
						echo $val_arr[0][0];
						echo '<br><p style="font-size:15px; font-weight: 400; margin: 0px 0px 0px 30px;">' . $val_arr[0][1] . '</p>';
					}
				 ?>
			</td>
		</tr>
						...
						<td>
							<?php
								if(!is_array($val)){
									...
									echo $comparison_data[$index_i][1][$index_j][1];
								}
