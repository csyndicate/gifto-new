...
if (!\class_exists('ShopMagicTwilioVendor\\WPDesk_Tracker_Sender_Logged')) {
    class WPDesk_Tracker_Sender_Logged implements \WPDesk_Tracker_Sender
    {
        ...
        {
            if (\class_exists('ShopMagicTwilioVendor\\WPDesk_Logger_Factory')) {
                \ShopMagicTwilioVendor\WPDesk_Logger_Factory::log_message("Sender payload: " . \json_encode($payload), self::LOGGER_SOURCE, \ShopMagicTwilioVendor\WPDesk_Logger::DEBUG);
                try {
                    $response = $this->sender->send_payload($payload);
                    \ShopMagicTwilioVendor\WPDesk_Logger_Factory::log_message('Sender response: ' . \json_encode($response), self::LOGGER_SOURCE, \ShopMagicTwilioVendor\WPDesk_Logger::DEBUG);
                    return $response;
                } catch (\ShopMagicTwilioVendor\WPDesk_Tracker_Sender_Exception_WpError $exception) {
                    \ShopMagicTwilioVendor\WPDesk_Logger_Factory::log_exception($exception);
                    throw $exception;
                }
            } else {
