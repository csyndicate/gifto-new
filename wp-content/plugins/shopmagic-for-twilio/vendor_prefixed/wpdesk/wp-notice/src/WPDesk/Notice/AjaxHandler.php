...
namespace ShopMagicTwilioVendor\WPDesk\Notice;

use ShopMagicTwilioVendor\WPDesk\PluginBuilder\Plugin\HookablePluginDependant;
use ShopMagicTwilioVendor\WPDesk\PluginBuilder\Plugin\PluginAccess;
/**
 * Class AjaxHandler
 *
 ...
 */
class AjaxHandler implements \ShopMagicTwilioVendor\WPDesk\PluginBuilder\Plugin\HookablePluginDependant
{
    use PluginAccess;
    ...
    public function processAjaxNoticeDismiss()
    {
        if (isset($_POST[self::POST_FIELD_NOTICE_NAME])) {
            ...
            $option_name = \ShopMagicTwilioVendor\WPDesk\Notice\PermanentDismissibleNotice::OPTION_NAME_PREFIX . $noticeName;
            if (\wp_verify_nonce($security, $option_name)) {
                \update_option($option_name, \ShopMagicTwilioVendor\WPDesk\Notice\PermanentDismissibleNotice::OPTION_VALUE_DISMISSED);
