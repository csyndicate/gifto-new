...
if (\PHP_VERSION_ID > 50300) {
    ...
    if (!isset($plugin_init_factory)) {
        $plugin_init_factory = new \ShopMagicTwilioVendor\WPDesk\Plugin\Flow\Initialization\Simple\SimpleFactory();
    }
    if (!isset($plugin_shops) || !\is_array($plugin_shops)) {
        $plugin_shops = array();
    }
    $bootstrap = new \ShopMagicTwilioVendor\WPDesk\Plugin\Flow\PluginBootstrap(
        $plugin_version,
        null,
        // deprecated
        $plugin_name,
