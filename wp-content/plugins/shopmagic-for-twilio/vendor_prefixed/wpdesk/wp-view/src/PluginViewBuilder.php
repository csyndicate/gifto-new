...
namespace ShopMagicTwilioVendor\WPDesk\View;

use ShopMagicTwilioVendor\WPDesk\View\Renderer\SimplePhpRenderer;
use ShopMagicTwilioVendor\WPDesk\View\Resolver\ChainResolver;
use ShopMagicTwilioVendor\WPDesk\View\Resolver\DirResolver;
use ShopMagicTwilioVendor\WPDesk\View\Resolver\WPThemeResolver;
/**
...
class PluginViewBuilder
{
    ...
    {
        $resolver = new \ShopMagicTwilioVendor\WPDesk\View\Resolver\ChainResolver();
        $resolver->appendResolver(new \ShopMagicTwilioVendor\WPDesk\View\Resolver\WPThemeResolver(\basename($this->plugin_dir)));
        foreach ($this->template_dirs as $dir) {
            $dir = \trailingslashit($this->plugin_dir) . \trailingslashit($dir);
            $resolver->appendResolver(new \ShopMagicTwilioVendor\WPDesk\View\Resolver\DirResolver($dir));
        }
        return new \ShopMagicTwilioVendor\WPDesk\View\Renderer\SimplePhpRenderer($resolver);
    }
    /**
