...
use ShopMagicTwilioVendor\Twilio\Exceptions\ConfigurationException;
use ShopMagicTwilioVendor\Twilio\Exceptions\TwilioException;
use ShopMagicTwilioVendor\Twilio\Http\Client as HttpClient;
use ShopMagicTwilioVendor\Twilio\Http\CurlClient;
use ShopMagicTwilioVendor\Twilio\Rest\Api;
use ShopMagicTwilioVendor\Twilio\Security\RequestValidator;
use ShopMagicTwilioVendor\Twilio\VersionInfo;
/**
...
class BaseClient
{
     ...
     */
    public function __construct(string $username = null, string $password = null, string $accountSid = null, string $region = null, \ShopMagicTwilioVendor\Twilio\Http\Client $httpClient = null, array $environment = null, array $userAgentExtensions = null)
    {
        ...
        if (!$this->username || !$this->password) {
            throw new \ShopMagicTwilioVendor\Twilio\Exceptions\ConfigurationException('Credentials are required to create a Client');
        }
        $this->accountSid = $accountSid ?: $this->username;
        if ($httpClient) {
            $this->httpClient = $httpClient;
        } else {
            $this->httpClient = new \ShopMagicTwilioVendor\Twilio\Http\CurlClient();
        }
    }
    /**
     ...
     */
    public function request(string $method, string $uri, array $params = [], array $data = [], array $headers = [], string $username = null, string $password = null, int $timeout = null) : \ShopMagicTwilioVendor\Twilio\Http\Response
    {
        $username = $username ?: $this->username;
        $password = $password ?: $this->password;
        $logLevel = \getenv('DEBUG_HTTP_TRAFFIC') === 'true' ? 'debug' : $this->getLogLevel();
        $headers['User-Agent'] = 'twilio-php/' . \ShopMagicTwilioVendor\Twilio\VersionInfo::string() . ' (' . \php_uname("s") . ' ' . \php_uname("m") . ')' . ' PHP/' . \PHP_VERSION;
    ...
    public function buildUri(string $uri) : string
    {
        ...
        return \ShopMagicTwilioVendor\Twilio\Security\RequestValidator::unparse_url($parsedUrl);
    }
    /**
    ...
    public function __get(string $name)
    {
        ...
        throw new \ShopMagicTwilioVendor\Twilio\Exceptions\TwilioException('Unknown domain ' . $name);
    }
    /**
     ...
     */
    public function validateSslCertificate(\ShopMagicTwilioVendor\Twilio\Http\CurlClient $client) : void
    {
        $response = $client->request('GET', 'https://tls-test.twilio.com:443');
        if ($response->getStatusCode() < 200 || $response->getStatusCode() > 300) {
            throw new \ShopMagicTwilioVendor\Twilio\Exceptions\TwilioException('Failed to validate SSL certificate');
        }
    }
    /**
     ...
     */
    public function getAccount() : \ShopMagicTwilioVendor\Twilio\Rest\Api\V2010\AccountContext
    {
     ...
     */
    public function getHttpClient() : \ShopMagicTwilioVendor\Twilio\Http\Client
    {
     ...
     */
    public function setHttpClient(\ShopMagicTwilioVendor\Twilio\Http\Client $httpClient) : void
    {
        $this->httpClient = $httpClient;
    }
    /**
