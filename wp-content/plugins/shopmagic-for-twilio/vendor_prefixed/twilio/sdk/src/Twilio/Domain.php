...
abstract class Domain
{
     ...
     */
    public function request(string $method, string $uri, array $params = [], array $data = [], array $headers = [], string $user = null, string $password = null, int $timeout = null) : \ShopMagicTwilioVendor\Twilio\Http\Response
    {
        $url = $this->absoluteUrl($uri);
        return $this->client->request($method, $url, $params, $data, $headers, $user, $password, $timeout);
    }
    public function getClient() : \ShopMagicTwilioVendor\Twilio\Rest\Client
    {
        return $this->client;
    }
