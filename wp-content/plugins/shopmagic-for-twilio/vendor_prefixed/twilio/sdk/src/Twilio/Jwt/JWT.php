...
class JWT
{
     ...
     */
    public static function decode(string $jwt, string $key = null, bool $verify = \true)
    {
        $tks = \explode('.', $jwt);
        if (\count($tks) !== 3) {
    ...
    public static function sign(string $msg, string $key, string $method = 'HS256') : string
    {
        ...
        return \hash_hmac($methods[$method], $msg, $key, \true);
    }
    /**
    ...
    {
        $messages = [\JSON_ERROR_DEPTH => 'Maximum stack depth exceeded', \JSON_ERROR_CTRL_CHAR => 'Unexpected control character found', \JSON_ERROR_SYNTAX => 'Syntax error, malformed JSON'];
