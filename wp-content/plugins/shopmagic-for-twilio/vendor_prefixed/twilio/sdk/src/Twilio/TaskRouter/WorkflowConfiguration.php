...
namespace ShopMagicTwilioVendor\Twilio\TaskRouter;
...
class WorkflowConfiguration implements \JsonSerializable
{
    ...
    public static function fromJson(string $json) : \ShopMagicTwilioVendor\Twilio\TaskRouter\WorkflowConfiguration
    {
        ...
        foreach ($configJSON->task_routing->filters as $filter) {
            ...
            $filter = new \ShopMagicTwilioVendor\Twilio\TaskRouter\WorkflowRule($filter->expression, $filter->targets, $friendly_name);
            $filters[] = $filter;
        }
        return new \ShopMagicTwilioVendor\Twilio\TaskRouter\WorkflowConfiguration($filters, $default_filter);
    }
