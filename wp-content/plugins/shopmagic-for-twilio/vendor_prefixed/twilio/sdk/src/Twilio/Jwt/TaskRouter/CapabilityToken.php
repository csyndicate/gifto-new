...
namespace ShopMagicTwilioVendor\Twilio\Jwt\TaskRouter;

use ShopMagicTwilioVendor\Twilio\Jwt\JWT;
/**
...
class CapabilityToken
{
    ...
    protected $required = ['required' => \true];
    protected $optional = ['required' => \false];
    public function __construct(string $accountSid, string $authToken, string $workspaceSid, string $channelId, string $resourceUrl = null, string $overrideBaseUrl = null, string $overrideBaseWSUrl = null)
    {
    ...
    public function addPolicyDeconstructed(string $url, string $method, ?array $queryFilter = [], ?array $postFilter = [], bool $allow = \true) : \ShopMagicTwilioVendor\Twilio\Jwt\TaskRouter\Policy
    {
        $policy = new \ShopMagicTwilioVendor\Twilio\Jwt\TaskRouter\Policy($url, $method, $queryFilter, $postFilter, $allow);
        $this->policies[] = $policy;
        return $policy;
    }
    public function allow(string $url, string $method, ?array $queryFilter = [], ?array $postFilter = []) : void
    {
        $this->addPolicyDeconstructed($url, $method, $queryFilter, $postFilter, \true);
    }
    ...
    {
        $payload = ['version' => $this->version, 'friendly_name' => $this->friendlyName, 'iss' => $this->accountSid, 'exp' => \time() + $ttl, 'account_sid' => $this->accountSid, 'channel' => $this->channelId, 'workspace_sid' => $this->workspaceSid];
        if (\strpos($this->channelId, 'WK') === 0) {
            $payload['worker_sid'] = $this->channelId;
        } else {
            if (\strpos($this->channelId, 'WQ') === 0) {
        ...
        return \ShopMagicTwilioVendor\Twilio\Jwt\JWT::encode($payload, $this->authToken, 'HS256');
    }
}
