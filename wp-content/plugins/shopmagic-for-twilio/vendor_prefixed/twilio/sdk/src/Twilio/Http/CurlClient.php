...
use ShopMagicTwilioVendor\Twilio\Exceptions\EnvironmentException;
class CurlClient implements \ShopMagicTwilioVendor\Twilio\Http\Client
{
    ...
    public function request(string $method, string $url, array $params = [], array $data = [], array $headers = [], string $user = null, string $password = null, int $timeout = null) : \ShopMagicTwilioVendor\Twilio\Http\Response
    {
        ...
        try {
            if (!($curl = \curl_init())) {
                throw new \ShopMagicTwilioVendor\Twilio\Exceptions\EnvironmentException('Unable to initialize cURL');
            }
            if (!\curl_setopt_array($curl, $options)) {
                throw new \ShopMagicTwilioVendor\Twilio\Exceptions\EnvironmentException(\curl_error($curl));
            }
            ...
            $this->lastResponse = new \ShopMagicTwilioVendor\Twilio\Http\Response($statusCode, $body, $responseHeaders);
            ...
            case 'put':
                ...
                if ($data) {
                    if ($buffer = \fopen('php://memory', 'w+')) {
                        ...
                        throw new \ShopMagicTwilioVendor\Twilio\Exceptions\EnvironmentException('Unable to open a temporary file');
                    }
                }
                break;
            case 'head':
    ...
    private function buildMultipartOptions(array $data) : array
    {
        ...
        foreach ($data as $key => $value) {
            if ($value instanceof \ShopMagicTwilioVendor\Twilio\Http\File) {
                ...
                $body .= \vsprintf("--%s\r\nContent-Disposition: form-data; name=\"%s\"; filename=\"%s\"\r\n%s\r\n%s\r\n", [$delimiter, $key, $filename, $headers, $chunk]);
            } else {
                $body .= \vsprintf("--%s\r\nContent-Disposition: form-data; name=\"%s\"\r\n\r\n%s\r\n", [$delimiter, $key, $value]);
            }
        }
        $body .= "--{$delimiter}--\r\n";
        return [["Content-Type: multipart/form-data; boundary={$delimiter}", 'Content-Length: ' . \strlen($body)], $body];
    }
}
