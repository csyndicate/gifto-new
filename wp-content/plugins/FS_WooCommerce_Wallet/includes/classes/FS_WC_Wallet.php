...
class FS_WC_Wallet {
	...
	// Array of notification and errors to display
	...
	function __construct() {
		...
		if ( ! in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
			$this->notices[] = __( 'WooCommerce need to be installed and activated to be able to use "WooCommerce Wallet".',
				'fsww' );
		}
		...
		if ( get_option( 'fsww_disable_payment_methods_except_for_top_up', 'off' ) == 'on' ) {
			add_filter( 'woocommerce_available_payment_gateways', array(
				$this,
				'disable_payment_methods_except_for_top_ups'
			) );
		}
		...
		if ( isset( $_POST['fsww_balance_to_add'] ) && ! empty( $_POST['fsww_balance_to_add'] ) ) {
			...
			if ( $is_valid['is_valid'] ) {
				add_filter( 'woocommerce_add_cart_item_data', array(
					$this,
					'add_fsww_product_price_to_cart_item_data'
				),
					10, 2 );
				...
				if ( $product ) {
					...
					$redirect_url = apply_filters( 'fsww_redirect_to_checkout_after_added_amount',
						true ) ? wc_get_checkout_url() : wc_get_cart_url();
	...
	/**
	 * Make top up product purchasable
	 *
	...
	/**
	 * Set top up product price at run time
	 *
	 * @param OBJECT $cart
	 *
	 * @return NULL
	...
	public function add_fsww_product_price_to_cart_item_data( $cart_item_data, $product_id ) {
		...
		if ( isset( $_POST['fsww_balance_to_add'] ) && $product ) {
			$recharge_amount                   = apply_filters( 'fsww_rechargeable_amount', $_POST['fsww_balance_to_add'] );
	...
	private static function create_recharge_product() {
		$product_args = array(
			...
			'post_type'    => 'product',
			'post_excerpt' => '',
			'post_content' => stripslashes( html_entity_decode( 'Auto generated product for wallet recharge please do not delete or update.',
				ENT_QUOTES, 'UTF-8' ) ),
			'post_author'  => 1
		);
	...
	/**
	 * Disable payment methods if wallet only mode is enabled
	 *
	 ...
	 * @return mixed
	 */
	public function disable_payment_methods_except_for_top_ups( $available_gateways ) {
		if ( ! is_admin() ) {
			$top_up_product_found = false;
			$normal_product_found = false;

			$wallet_gateway = array();
			$other_gateways = array();

			foreach ( $available_gateways as $key => $value ) {
				if ( $key != 'fsww' ) {
					$other_gateways[ $key ] = $value;
				} else {
					$wallet_gateway[ $key ] = $value;
				}
			}
			...
			if ( $normal_product_found && $top_up_product_found ) {
				add_filter( 'woocommerce_no_available_payment_methods_message', array(
					$this,
					'cant_mix_products_notice'
				), 10, 1 );

				return array();
			} elseif ( $top_up_product_found && ! $normal_product_found ) {
	...
	public function cant_mix_products_notice( $notice ) {
		return esc_html__( "You can't mix top-up products and normal products in one order, please remove one of them.", 'fsww' );
	}
	...
	public function run_functions() {
		...
		if ( isset( $_POST['fsww-request-withrawal'] ) && $_POST['fsww-request-withrawal'] == "fsww" ) {
			$this->process_withdrawal_request();
		}
	...
	public function fsww_withdraw_callback() {
		...
		$request    = $wpdb->get_row( "SELECT * FROM {$wpdb->prefix}fswcwallet_withdrawal_requests WHERE request_id = $request_id" );

		$status = 'accepted';

		$fee = 0;
		if ( $request->fee !== null ) {
			Wallet::remove_unavailable_funds( $request->user_id, ( $request->amount + $request->fee ) );
			$fee = $request->fee;
		} else {
			$type   = get_option( 'fsww_fee_withdrawals_type', 'percentage' );
			$charge = get_option( 'fsww_fee_withdrawals_amount', '0' );

			if ( $type == 'percentage' ) {
				$fee = ( $request->amount / 100 ) * $charge;
			} else {
				$fee = (int) $charge;
			}
		}

		Wallet::withdraw_funds( $request->user_id, $request->amount, 0, __( 'Withdrawal request', 'fsww' ) );

		if ( $fee > 0 ) {
			Wallet::withdraw_funds( $request->user_id, $fee, 0, __( 'Withdrawal fee', 'fsww' ) );
		}
	...
	public function process_send_money() {
		...
		$type   = get_option( 'fsww_fee_transfers_type', 'percentage' );
		$charge = get_option( 'fsww_fee_transfers_amount', '0' );
		...
		if ( ! $errors ) {
			...
			Wallet::withdraw_funds( $sender_id, $fee, 0, __( "Transfer fee", "fsww" ) );
	...
	function process_withdrawal_request() {
		...
		$min_amount = get_option( 'fsww_withdrawal_input_min', '1' );
		$max_amount = get_option( 'fsww_withdrawal_input_max', '10000' );
		...
		$lock_status = Wallet::lock_status( $user_id );
		...
		if ( $lock_status['status'] != 'locked' ) {
				...
				if ( $_POST['method'] == "bitcoin" ) {
					...
					if ( $_POST['method'] == "swift" ) {
						...
						if ( $_POST['method'] == "bank" ) {
							...
							if ( $_POST['method'] == "bank_turkey" ) {

								$method = "Bank Transfer (Turkey)";
								$adress = json_encode( $_POST["bank_turkey"] );

							} else {
								if ( $_POST['method'] == "bank-ita" ) {

									$method = "Bank Transfer (Italy)";
									$adress = json_encode( $_POST["bank-ita"] );

								} else {
									if ( $_POST['method'] == "bank-bacs" ) {

										$method = "Bank Transfer (BACS)";
										$adress = json_encode( $_POST["bank-bacs"] );
			...
			if ( $balance >= ( $amount + $fee ) ) {
				...
				if ( ! $errors ) {
					...
					Wallet::add_unavailable_funds( $user_id, ( $amount + $fee ) );
	...
	/**
	 * Add request refund button - order details page
	 *
	 * @param $order
	 */
	function add_refund_request_button_order_page( $order ) {
		...
		if ( $status == 'requested' ) {
			...
			echo '<a href="' . admin_url( 'admin-ajax.php?action=fsww_crr&order_id=' . $order->get_id() ) . '" class="button">' . __( 'Cancel Refund Request',
					'fsww' ) . '</a>';
			...
			if ( in_array( $order->get_status(), array( 'completed', 'processing' ) ) ) {
				echo '<a href="' . admin_url( 'admin-ajax.php?action=fsww_rr&order_id=' . $order->get_id() ) . '" class="woocommerce-button button fsww-request-refund ">' . __( 'Request Refund',
						'fsww' ) . '</a>';
			}
	...
	/**
	 * dd request refund button - orders table page
	 *
	 * @param $actions
	 * @param $order
	 *
	...
	function make_refund_request_callback() {
		...
		$data = array(
			...
			'status'       => $status,
			'reason'       => sanitize_text_field( strip_tags( $_POST['fsww_refund_reason'] ) )
		);
	...
	/**
	 * Redirect to checkout if its a wallet credit product
	 *
	 * @return string
	 */
	...
	function custom_pre_get_posts_query( $q ) {
		...
		$tax_query[] = array(
			...
			// Don't display products in the carton category on the shop page.
			'operator' => 'NOT IN'
		);
	...
	public function add_plugin_actions() {
		if ( current_user_can( 'manage_woocommerce' ) ) {
			...
			add_action( 'wp_ajax_fsww_export_transactions', array( $this, 'fsww_export_transactions_callback' ) );
			add_action( 'wp_ajax_fsww_export_withdrawal', array( $this, 'fsww_export_withdrawal_callback' ) );
	 ...
	 * Export transactions
	 */
	public function fsww_export_transactions_callback() {
		header( 'Content-Type: application/csv' );
		header( 'Content-Disposition: attachement; filename="wallet-transactions-' . date( "Y-m-d-His" ) . '.csv";' );
		...
		$head = '"transaction_id","order_id","user_id","type","transaction_date","amount","action_performed_by","transaction_description"';

		$query = $wpdb->get_results( "SELECT * FROM 
                                            {$wpdb->prefix}fswcwallet_transaction 
                                        ORDER BY transaction_id ASC", ARRAY_A );
		...
		if ( $query ) {
			...
			foreach ( $query as $row ) {
				...
				$export['transaction_id'] = $row['transaction_id'];
				$export['order_id']       = $row['order_id'];
				...
				$export['transaction_date']        = $row['transaction_date'];
				$export['amount']                  = $row['amount'];
				$export['action_performed_by']     = $row['action_performed_by'];
				$export['transaction_description'] = strip_tags( $row['transaction_description'] );
	 ...
	 */
	public function fsww_export_withdrawal_callback() {
		header( 'Content-Type: application/csv' );
		header( 'Content-Disposition: attachement; filename="wallet-withdrawal-' . date( "Y-m-d-His" ) . '.csv";' );
		...
		$head = '"request_id","user_id","amount","payment_method","status","address"';

		$query = $wpdb->get_results( "SELECT * FROM 
                                            {$wpdb->prefix}fswcwallet_withdrawal_requests 
                                        ORDER BY request_id ASC", ARRAY_A );
		...
		if ( $query ) {
			...
			foreach ( $query as $row ) {
				...
				$export['request_id']     = $row['request_id'];
				$export['user_id']        = $row['user_id'];
				$export['amount']         = $row['amount'];
				$export['payment_method'] = $row['payment_method'];
				$export['status']         = $row['status'];
				$export['address']        = str_replace( array( "stdClass Object\n" ), '',
					print_r( json_decode( $row['address'] ), true ) );
	...
	public function fsww_export_callback() {
		...
		header( 'Content-Type: application/csv' );
		header( 'Content-Disposition: attachement; filename="wallets-' . date( "Y-m-d-His" ) . '.csv";' );
	...
	public function generate_csv() {
		...
		$query = $wpdb->get_results( "SELECT ID, user_email, user_login FROM {$wpdb->prefix}users ORDER BY ID ASC",
			ARRAY_A );
	...
	public function fsww_refund_callback() {
		...
		foreach ( $fees as $fee ) {
			$key = $fee->get_meta( 'fsww_partial_payment' );

			if ( $key == '_fsww_partial_payment' ) {
	...
	public function fsww_edit_wallet_callback() {
		...
		//  $unavailable_funds = FS_WC_Wallet::encrypt_decrypt('encrypt', $_POST['unavailable_funds']);
		...
		$data = array(
			...
			// 'unavailable_funds' => $unavailable_funds
		);
	...
	public function fsww_i_add_funds_request() {
		...
		if ( $notify == 'on' ) {
			...
			$message = '<p>' . __( 'We have added', 'fsww' ) . ' ' . fsww_price( $amount ) . ' ' . __( 'to your Wallet balance',
					'fsww' ) . '</p><br>' . '<p>'.'Yay! Please go ahead and Gift away to your loved once or to yourself at Gifto.co' .'<p>'. $message;
			$subject = __( 'We have added funds to your Wallet!', 'fsww' );
			$heading = __( 'We have added funds to your Wallet!', 'fsww' );
		...
		Wallet::add_funds( $user_id, $amount, 0,
			( __( "Admin added funds", "fsww" ) . ( $message != '' ? ( ', ' . $message ) : '' ) ) );
	...
	public function fsww_i_withdraw_request() {
		...
		if ( $notify == 'on' ) {
			...
			$message = '<p>' . fsww_price( $amount ) . ' ' . __( 'Have been withdrawn from your account balance',
					'fsww' ) . '</p><br>' . $message;
			$subject = __( 'Funds have been withdrawn from your account', 'fsww' );
			$heading = __( 'Funds have been withdrawn from your account', 'fsww' );

			$this->send_email( $user_id, $message, $subject, $heading );

		}

		Wallet::withdraw_funds( $user_id, $amount, 0,
			( __( "Admin withdraw funds", "fsww" ) . ( $message != '' ? ( ', ' . $message ) : '' ) ) );
	...
	public function fsww_i_lock_request() {
		...
		if ( $action == 'lock' ) {
			...
			if ( $notify == 'on' ) {
				...
				$message = '<p>' . __( 'Your wallet balance have been locked, you can no longer use it to pay for products.',
						'fsww' ) . '</p><br>' . $message;
	...
	public function add_actions() {
		...
		add_action( 'wp_footer', array( $this, 'add_plugin_html' ) );
		...
		add_filter( 'woocommerce_product_query_tax_query', array(
			$this,
			'exclude_product_category_in_tax_query'
		), 10, 2 );
		add_filter( 'woocommerce_product_categories_widget_args', array(
			$this,
			'exclude_product_categories_widget'
		), 10,
			1 );
		...
		if ( get_option( 'fsww_partial_payments', 'on' ) == 'on' ) {
			...
			add_filter( 'woocommerce_cart_totals_get_fees_from_cart_taxes',
				array( $this, 'woocommerce_cart_totals_get_fees_from_cart_taxes' ), 10, 2 );
		...
		add_action( 'woocommerce_order_refunded', array( $this, 'manual_refund' ), 10, 2 );
		...
		add_action( 'woocommerce_process_product_meta_variable', array(
			$this,
			'woo_add_custom_variable_fields_save'
		), 1 );

		add_action( 'woocommerce_ajax_save_product_variations', array(
			$this,
			'woo_add_custom_variable_fields_save_ajax'
		),
			10, 2 );
		...
		add_action( 'woocommerce_after_order_fee_item_name', array( $this, 'partial_payment_admin_notice' ), 10, 3 );
	}

	public function create_order_fee_item( $item, $fee_key, $fee, $order ) {
		$item->add_meta_data( 'fsww_partial_payment', $fee_key, true );
	}
	...
	public function order_cancelled( $order_id ) {
		...
		if ( $order->get_payment_method() != 'fsww' ) {
			if ( add_post_meta( $order_id, 'fswcw_refunded', 'true', true ) ) {
				...
				if ( $transaction_found ) {
					Wallet::refund( $order->get_user_id(), $total_fees, $order_id, __( 'Partial payment refund', 'fsww' ), true );
				}
			}
		}
	}

	public function partial_payment_admin_notice( $item_id, $fee ) {
		$key = $fee->get_meta( 'fsww_partial_payment' );

		if ( $key == '_fsww_partial_payment' ) {
			echo '<span class="fsww-pp-info">' . __( 'Partial payment refunds has to be added to the customers\' wallet manually, unless you are refunding by changing the order status.', 'fsww' ) . '</span>';
		}
	}

	public function manual_refund( $order_id, $refund_id ) {
		require_once( dirname( __FILE__ ) . '/Wallet.php' );
		...
		if ( $order->get_payment_method() == 'fsww' ) {
			foreach ( $order_refunds as $refund ) {
				if ( $refund->get_id() === $refund_id ) {
					Wallet::refund( $user_id, abs( $refund->get_total() ), $order_id, __( 'Order refund', 'fsww' ) );
				}
			}
		}

		if ( $order->get_payment_method() != 'fsww' && ( $order->get_status() == 'refunded' || $order->get_status() == 'Cancelled' ) ) {
			if ( add_post_meta( $order_id, 'fswcw_refunded', 'true', true ) ) {
				$fees              = $order->get_fees();
				$total_fees        = 0;
				$transaction_found = false;
				...
				if ( $transaction_found ) {
					Wallet::refund( $user_id, $total_fees, $order_id, __( 'Partial payment refund', 'fsww' ) );
				}
			}
		}
	}
	...
	public function user_registration( $user_id ) {
		...
		$amount = (float) get_option( 'fsww_registration_reward', '0' );

		if ( $amount > 0 && ! Wallet::wallet_exist( $user_id ) ) {
			Wallet::add_funds( $user_id, $amount, 0, __( 'Registration Reward' ) );
		}
	}

	/**
	 * Add custom html to the frontend.
	 */
	public function add_plugin_html() {
		require_once FSWW_BASE . '/includes/frontend-html.php';
	}

	/**
	 * Transactions page printable view.
	 *
	 * @return bool
	 */
	...
	/**
	 * Add partial payments - checkout page.
	 */
	function partial_payment_add_remove_checkout() {
		...
		if ( is_user_logged_in() ) {
			...
			foreach ( $woocommerce->cart->get_cart() as $key => $value ) {
				...
				if ( get_option( 'fsww_disable_on_sale', 'off' ) == 'on' ) {
			...
			if ( ( $balance < $total && $balance > 0 ) && $no_recharge_products ) {
				...
				if ( self::$remove_partial_payment == '0' ) {
					...
					echo '	  <a href="' . $cart_page_url . '?fsww_remove_partial_payment=1" class="button">' . __( 'Remove',
							'fsww' ) . '</a> ' . __( 'Would you like to remove wallet partial payment?', 'fsww' );
					echo '</div>';

				} else {
					if ( self::$remove_partial_payment == '1' ) {

						echo '<div class="woocommerce-message">';
						echo '	  <a href="' . $cart_page_url . '?fsww_remove_partial_payment=0" class="button">' . __( 'Add',
								'fsww' ) . '</a> ' . __( 'You have ',
								'fsww' ) . fsww_price( $balance ) . __( ' would you like to use them?', 'fsww' );
						echo '</div>';
	...
	/**
	 * Add partial payments - cart page.
	 */
	...
	function woo_add_custom_general_fields() {
		...
		woocommerce_wp_text_input(
			array(
				...
				'placeholder' => '',
				'desc_tip'    => 'true',
				'description' => __( 'Enter the wallet cashback amount given after purchasing this product<br><br>Enter an exact value, or a value ending with % to give a percentage or leave empty for no cashback',
					'fsww' ),
	...
	public function custom_wc_add_fee( $cart ) {
		...
		foreach ( $woocommerce->cart->get_cart() as $cart_item_key => $cart_item ) {
			...
			if ( $cart_item['product_id'] == get_option( '_fsww_recharge_product' ) ) {
				return false;
			}
		...
		$partial_payment_amount = Wallet::get_balance( get_current_user_id() );
		$total                  = floatval( $woocommerce->cart->subtotal );

		$fee = array(
			'id'        => '_fsww_partial_payment',
			'name'      => __( 'Paid From Wallet', 'fsww' ),
			'amount'    => (float) - 1 * $partial_payment_amount,
			'taxable'   => false,
			'tax_class' => 'fsww_pfw',
		);

		if ( ( $total > $partial_payment_amount ) && ( $partial_payment_amount > 0 ) ) {
			if ( self::$remove_partial_payment == '0' ) {
	...
	public function add_credits_to_user_account( $order_id ) {
		...
		if ( add_post_meta( $order_id, 'fsww_loaded', 'true', true ) ) {
			...
			$type   = get_option( 'fsww_fee_topup_type', 'percentage' );
			$charge = get_option( 'fsww_fee_topup_amount', '0' );


			$order            = wc_get_order( $order_id );
			$topup_product_id = get_option( '_fsww_recharge_product' );
			...
			if ( count( $order->get_items() ) > 0 ) {
				...
				foreach ( $order->get_items() as $item ) {
					...
					if ( get_option( 'fsww_deposit_input', 'off' ) == 'on' ) {
						...
						if ( $wallet_product ) {
							if ( $wallet_product->get_id() == $product_id ) {
								...
								if ( $fee > 0 ) {
									Wallet::withdraw_funds( $order->get_user_id(), $fee, 0, __( 'Top up fee', 'fsww' ) );
								}
					...
					if ( $topup_product_id != $item['product_id'] && ( $loaded == false || get_option( 'fsww_deposit_input',
								'off' ) == 'on' || get_option( 'fsww_show_both_options', '' ) == 'on' ) ) {
	...
	public function add_shortcodes() {
		...
		add_shortcode( 'fsww_withdraw', array( $this, 'shortcode_make_withdraw' ) );

		add_shortcode( 'fsww_lock_status', array( $this, 'shortcode_lock_status' ) );

	}


	public function shortcode_lock_status( $sc_args ) {
		...
		if ( $lock_status['status'] == 'locked' ) {
			echo __( 'Your wallet is locked', 'fsww' );
		} else {
			echo __( 'Your wallet is not locked', 'fsww' );
		}
	}

	public function shortcode_make_withdraw( $sc_args ) {
		...
		require_once( dirname( __FILE__ ) . '/../withdrawal-requests.php' );
	...
	public function shortcode_send_money( $sc_args ) {
		...
		if ( isset( $_GET['self'] ) ) {
			...
			echo "<div class=\"woocommerce-Message woocommerce-Message--error woocommerce-error\">" . __( "You can't send money to yourself.",
					"fsww" ) . "</div>";
		...
		if ( isset( $_GET['email'] ) ) {
			...
			echo "<div class=\"woocommerce-Message woocommerce-Message--error woocommerce-error\">" . __( "Invalid user email.",
					"fsww" ) . "</div>";
		...
		if ( isset( $_GET['amount'] ) ) {
			...
			echo "<div class=\"woocommerce-Message woocommerce-Message--error woocommerce-error\">" . __( "Insufficient funds",
					"fsww" ) . "</div>";
		...
		if ( isset( $_GET['success'] ) ) {
			...
			echo "<div class=\"woocommerce-message\">" . __( "The amount have been sent.",
					"fsww" ) . "</div>";
        ...
        <div class="fsww-meke-deposit-sc">
            ...
            <form id="send_money" method="post">
				...
				<p class="form-row form-row-wide">
					...
					<input type="text" class="input-text fsww-input woocommerce-Input woocommerce-Input--text input-text" name="amount" id="amount" placeholder="0.00" value="" step="any" required>
				...
				<p class="form-row form-row-wide">
					...
					<input type="email" class="input-text fsww-input woocommerce-Input woocommerce-Input--text input-text" name="send_to" id="send_to" placeholder="" value="" required>
	...
	public function shortcode_make_deposit( $sc_args ) {
		...
		$select = '
            <form id="add_to_cart" method="post">
                ...
                <select class="fsww-select fsww-input woocommerce-select-control" name="add-to-cart">
                    ' . $options . '
                </select>
		...
		$input = ' <form method="post" action="">
                    <div class="woo-wallet-add-amount">
                        ...
                        <p class="woocommerce-form-row woocommerce-form-row--wide form-row form-row-wide">
                             <label for="fsww_balance_to_add">' . __( 'Enter amount', 'fsww' ) . '</label>
                         
                             <input type="number" step="any" value="' . $min . '" min="' . $min . '"  max="' . $max . '" name="fsww_balance_to_add" id="fsww_balance_to_add" class="input-text fsww-input woocommerce-Input woocommerce-Input--text input-text" />
		...
		if ( get_option( 'fsww_show_both_options', '' ) == 'on' ) {
			$output = $output . $select . $input . $end;
		} else {
			if ( get_option( 'fsww_deposit_input', 'off' ) == 'on' ) {
	...
	public function register_custom_setting() {
		...
		register_setting( 'fsww_general_options_group', 'fsww_disable_on_sale' );

		$menu_locations = get_nav_menu_locations();
		foreach ( $menu_locations as $key => $location ) {
			register_setting( 'fsww_general_options_group', 'fsww_show_balance_in_menu_' . $key );
		}
		...
		register_setting( 'fsww_general_options_group', 'fsww_withdrawals_bank_ita_transfer' );
		register_setting( 'fsww_general_options_group', 'fsww_withdrawals_bank_transfer_bacs' );
		...
		register_setting( 'fsww_general_options_group', 'fsww_withdrawal_input_min' );
		register_setting( 'fsww_general_options_group', 'fsww_withdrawal_input_max' );

		register_setting( 'fsww_general_options_group', 'fsww_registration_reward' );

		register_setting( 'fsww_general_options_group', 'fsww_disable_payment_methods' );
		register_setting( 'fsww_general_options_group', 'fsww_disable_payment_methods_except_for_top_up' );
		...
		register_setting( 'fsww_general_options_group', 'fsww_show_both_options' );


		register_setting( 'fsww_advanced_options_group', 'fsww_db_version' );


		register_setting( 'fsww_fees_options_group', 'fsww_fee_refund_type' );
		register_setting( 'fsww_fees_options_group', 'fsww_fee_refund_amount' );

		register_setting( 'fsww_fees_options_group', 'fsww_fee_withdrawals_type' );
		register_setting( 'fsww_fees_options_group', 'fsww_fee_withdrawals_amount' );

		register_setting( 'fsww_fees_options_group', 'fsww_fee_transfers_type' );
		register_setting( 'fsww_fees_options_group', 'fsww_fee_transfers_amount' );

		register_setting( 'fsww_fees_options_group', 'fsww_fee_topup_type' );
		register_setting( 'fsww_fees_options_group', 'fsww_fee_topup_amount' );
	...
	public function account_alerts() {
		...
		foreach ( $fees as $fee ) {
			if ( $fee->id == '_fsww_partial_payment ' ) {
				return true;
			}
		}
		...
		if ( get_option( 'fsww_refunds', 'on' ) == 'on' ) {
			...
			if ( get_option( 'fsww_refunds_button_position', 'table' ) == 'order_details' ) {
				add_filter( 'woocommerce_order_details_after_order_table',
					array( $this, 'add_refund_request_button_order_page' ), 100, 2 );
			}
	...
	function custom_price_message_variation( $data, $product, $variation ) {
		...
		if ( $cashback_value != '' ) {
			...
			$cashback = ' | ' . $cashback_value . __( ' Cashback', 'fsww' );
	...
	public function in_menu_balance( $items, $args ) {
		...
		if ( is_user_logged_in() ) {
			...
			if ( get_option( 'fsww_show_balance_in_menu_' . $args->theme_location, '' ) == 'on' ) {
				...
				$output = '<span class="fsww-wallet-icon fsww-icon-wallet"></span>' . $balance;
				...
				$items .= '<li class="menu-item menu-item-type-custom menu-item-object-custom fsww-balance ' . $right . '"><a href="' . wc_get_endpoint_url( 'make-a-deposit',
						'', $base ) . '">' . $output . '</a></li>';
	...
	public function register_styles() {
		wp_enqueue_style( 'FSWW_style', plugins_url( '/assets/styles/style.css', FSWW_FILE ), array(), '1.2' );
	}

	//Register scripts
	public function register_scripts() {
		wp_enqueue_script( 'FSWW_main', plugins_url( '/assets/scripts/main.js', FSWW_FILE ), array( 'jquery' ), '2.1' );
	...
	public function additional_custom_styles() {
		wp_enqueue_style( 'fsww_style', plugins_url( '/assets/styles/frontend.css', FSWW_FILE ), array(), '1.2' );
		wp_enqueue_script( 'fsww_script', plugins_url( '/assets/scripts/frontend.js', FSWW_FILE ), array( 'jquery' ), '1.2' );
	}
	...
	public static function set_encryption_key( $key, $vi, $action = 'set' ) {
		...
		if ( ! file_exists( $target_dir ) ) {
			...
			fwrite( $fp,
				"<?php define(\"FSWW_ENCRYPTION_KEY\", \"" . $key . "\");\ndefine(\"FSWW_ENCRYPTION_VI\", \"" . $vi . "\");" );
			fclose( $fp );
