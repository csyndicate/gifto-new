...
function transactions_table_html($user_id = '', $is_user = false, $page = 'fsww-t')
{
    ...
    if (!$is_user) { ?>
        <div class="pages">
            <a href="<?php echo admin_url("admin.php?page={$page}{$formated_user_id}&p=1"); ?>"><span
                        class="pagination-links"><span class="tablenav-pages-navspan button" aria-hidden="true">«</span></a>
            <a href="<?php echo admin_url("admin.php?page={$page}{$formated_user_id}&p={$previous_page}"); ?>"><span
                        class="tablenav-pages-navspan button" aria-hidden="true">‹</span></a>
            <span class="screen-reader-text"><?php _e('Current Page', 'fsww') ?></span><span id="table-paging"
                                                                                             class="paging-input"><?php echo $current_page ?> of <span
                        class="total-pages"><?php echo $total_pages ?></span></span>
            <a href="<?php echo admin_url("admin.php?page={$page}{$formated_user_id}&p={$next_page}"); ?>"><span
                        class="pagination-links"><span class="tablenav-pages-navspan button" aria-hidden="true">›</span></a>
            <a href="<?php echo admin_url("admin.php?page={$page}{$formated_user_id}&p={$total_pages}"); ?>"><span
                        class="tablenav-pages-navspan button" aria-hidden="true">»</span></span></a>
        </div>
    <?php } ?>

    <table class="fsww-container fsww-transactions wp-list-table widefat striped woocommerce-orders-table woocommerce-MyAccount-orders shop_table shop_table_responsive my_account_orders account-orders-table">
        <thead>
        <tr>
        ...
        if ($query) {
            ...
            foreach ($query as $transaction) {
                ...
                if ($order_id != 0 && $is_user == false) {
                    ...
                    $order_url = '<a target="_blank" href="' . admin_url("post.php?post={$transaction->order_id}&action=edit") . "\">#{$transaction->order_id} - " . __('View Order',
                            'fsww') . '</a>';
                    ...
                    $order = wc_get_order($order_id);
                    $order_url = '<a target="_blank" href="' . ($order ? $order->get_view_order_url() : '') . "\">#{$transaction->order_id} - " . __('View Order',
                            'fsww') . '</a>';
                ...
                <tr>
                    <td data-title="<?php echo __('ID', 'fsww') ?>">
                        <?php echo $transaction_id ?>
                        <div class="row-actions fsactions">
                            <?php if (!$is_user) { ?>
                                <span class="trash">
                                <a href="<?php echo admin_url("admin.php?page=fsww-t&task=delete&transaction_id={$transaction_id}") ?>"
                                   class="editinline fsww-delete"
                                   data-id="<?php echo $user_id ?>"><?php echo __('Delete', 'fsww'); ?></a>
                            </span>
                            <?php } ?>
                        </div>
                    </td>
                    <td data-title="<?php echo __('Order ID', 'fsww') ?>"><?php echo $order_url ?></td>
                    <?php if (!$is_user) { ?>
                        <td><?php echo $username ?></td>
                    <?php } ?>
                    <td data-title="<?php echo __('Amount', 'fsww') ?>"><?php echo $display_amount ?></td>
                    <td data-title="<?php echo __('Description', 'fsww') ?>"><?php echo $transaction_description ?></td>
                    <?php if (!$is_user) { ?>
                        <td><?php echo $action_performed_by ?></td>
                    <?php } ?>
                    <td data-title="<?php echo __('Date', 'fsww') ?>"><?php echo $date ?></td>
        ...
        if ($query) {
            ...
            foreach ($query as $transaction) {
                ...
                if ($order_id != 0 && $is_user == false) {
                    ...
                    $order = wc_get_order($order_id);
                    $order_url = '<a target="_blank" href="' . $order->get_view_order_url() . "\">#{$transaction->order_id} - " . __('View Order',
                            'fsww') . '</a>';
