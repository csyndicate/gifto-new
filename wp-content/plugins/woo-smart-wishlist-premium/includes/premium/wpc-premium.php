...
if ( ! function_exists( 'woosw_update_checker' ) ) {
	add_action( 'init', 'woosw_update_checker', 99 );
	add_action( 'admin_notices', 'woosw_update_notice', 99 );
	add_action( 'in_plugin_update_message-' . plugin_basename( WOOSW_FILE ), 'woosw_update_license', 99 );

	function woosw_update_checker() {
		if ( $key = wpc_get_update_key( 6514 ) ) {
			PucFactory::buildUpdateChecker( 'https://api.wpclever.net/update/' . $key . '.json', WOOSW_FILE, plugin_basename( WOOSW_DIR ) );
		} else {
			PucFactory::buildUpdateChecker( 'https://api.wpclever.net/update/6514.json', WOOSW_FILE, plugin_basename( WOOSW_DIR ) );
		}
	}

	function woosw_update_license( $plugin_data ) {
		if ( empty( $plugin_data['package'] ) ) {
			echo ' <em>Please verify your license key in <a href="' . admin_url( 'admin.php?page=wpclever-keys' ) . '" target="_blank">WPClever > License Keys</a> to update.</em>';
		}
	}

	function woosw_update_notice() {
		if ( apply_filters( 'wpc_dismiss_notices', false ) ) {
			return;
		}

		if ( get_option( 'wpc_dismiss_notice_woosw_update' ) ) {
			return;
		}

		if ( ! wpc_get_update_key( 6514 ) ) {
			?>
            <div data-dismissible="woosw_update" class="wpc-notice notice notice-warning is-dismissible">
                <p>Please verify <a href="<?php echo admin_url( 'admin.php?page=wpclever-keys' ); ?>">License Key</a> of
                    <strong>WPC Smart Wishlist</strong> to enjoy unlimited update release and get the latest plugin update directly on the website backend.
                </p>
            </div>
			<?php
		}
	...
	class WPCleverPremium {
		...
		function admin_enqueue_scripts( $hook ) {
			if ( strpos( $hook, 'wpclever-keys' ) ) {
				wp_enqueue_style( 'wpc-premium', WOOSW_URI . 'includes/premium/css/premium.css' );
				wp_enqueue_script( 'wpc-premium', WOOSW_URI . 'includes/premium/js/premium.js', [ 'jquery', ] );
			}
		}
            ...
            <div class="wpclever_page wpclever_update_keys_page wrap">
                ...
                <div class="card">
                    ...
                    <div class="wpclever_update_keys_form">
                        <input type="hidden" name="wpc_update_site" id="wpc_update_site" value="<?php echo esc_attr( get_bloginfo( 'url' ) ); ?>"/>
                        <input type="hidden" name="wpc_update_email" id="wpc_update_email" value="<?php echo esc_attr( get_bloginfo( 'admin_email' ) ); ?>"/>
