...
! defined( 'WOOST_VERSION' ) && define( 'WOOST_VERSION', '2.0.8' );
			...
			class WPCleverWoost {
				...
				function __construct() {
					...
					add_action( 'before_woocommerce_init', function () {
						if ( class_exists( '\Automattic\WooCommerce\Utilities\FeaturesUtil' ) ) {
							FeaturesUtil::declare_compatibility( 'custom_order_tables', WOOST_FILE );
						}
					} );
				}

				function admin_enqueue_scripts() {
					wp_enqueue_style( 'woost-backend', WOOST_URI . 'assets/css/backend.css', [], WOOST_VERSION );
				...
				function tab( $tab, $new = false ) {
					...
					if ( 'custom' === $tab['type'] ) {
						...
						<div class="woost-tab woost-tab-custom <?php echo esc_attr( $new ? 'active' : '' ); ?>">
							<div class="woost-tab-header">
								...
								<span class="woost-tab-label"><span class="woost-tab-title"><?php echo esc_html( $tab['title'] ); ?></span> <span class="woost-tab-label-type">#custom</span></span>
							...
							<div class="woost-tab-content">
								<div class="woost-tab-line">
									...
									<input type="hidden" value="<?php echo esc_attr( $tab['type'] ); ?>" name="woost_tabs[<?php echo esc_attr( $key ); ?>][type]"/>
									<input type="text" class="woost-tab-title-input" style="width: 100%" name="woost_tabs[<?php echo esc_attr( $key ); ?>][title]" placeholder="<?php esc_attr_e( 'Tab title', 'wpc-product-tabs' ); ?>" value="<?php echo esc_attr( $tab['title'] ); ?>" required/>
								</div>
								<div class="woost-tab-line">
									<?php
									if ( $new ) {
										...
										wp_editor( $content, $woost_editor_id, [
					...
					<?php } else { ?>
						<div class="<?php echo esc_attr( 'woost-tab woost-tab-' . $tab['type'] ); ?> <?php echo esc_attr( $new ? 'active' : '' ); ?>">
							<div class="woost-tab-header">
								<span class="woost-tab-move"><?php esc_html_e( 'move', 'wpc-product-tabs' ); ?></span>
								<span class="woost-tab-label"><span class="woost-tab-title"><?php echo esc_html( $tab['title'] ); ?></span> <span class="woost-tab-label-type">#<?php echo esc_attr( $tab['type'] ); ?></span></span>
								<span class="woost-tab-remove"><?php esc_html_e( 'remove', 'wpc-product-tabs' ); ?></span>
							</div>
							<div class="woost-tab-content">
								<div class="woost-tab-line">
									...
									<input type="hidden" value="auto" name="woost_tabs[<?php echo esc_attr( $key ); ?>][content]"/>
								</div>
							</div>
						</div>
						<?php
				...
				function new_tab() {
					...
					<div class="woost-tabs-new">
						...
						<input type="button" class="button woost-tab-new" value="<?php esc_attr_e( '+ Add new tab', 'wpc-product-tabs' ); ?>"/>
					</div>
					<?php
				}
