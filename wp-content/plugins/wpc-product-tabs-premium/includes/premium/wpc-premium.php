...
if ( ! function_exists( 'woost_update_checker' ) ) {
	add_action( 'init', 'woost_update_checker', 99 );
	add_action( 'admin_notices', 'woost_update_notice', 99 );

	function woost_update_checker() {
		if ( $key = wpc_get_update_key( 12537 ) ) {
			PucFactory::buildUpdateChecker( 'https://api.wpclever.net/update/' . $key . '.json', WOOST_FILE, plugin_basename( WOOST_DIR ) );
		}
	}

	function woost_update_notice() {
		if ( apply_filters( 'wpc_dismiss_notices', false ) ) {
			return;
		}

		if ( get_option( 'wpc_dismiss_notice_woost_update' ) ) {
			return;
		}
	...
	class WPCleverPremium {
		...
		function admin_enqueue_scripts( $hook ) {
			if ( strpos( $hook, 'wpclever-keys' ) ) {
				wp_enqueue_style( 'wpc-premium', WOOST_URI . 'includes/premium/css/premium.css' );
				wp_enqueue_script( 'wpc-premium', WOOST_URI . 'includes/premium/js/premium.js', [ 'jquery', ] );
			}
		}
