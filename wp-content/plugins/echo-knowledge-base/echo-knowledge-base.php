...
final class Echo_Knowledge_Base {
	...
	private function handle_ajax_requests( $action ) {
		...
		if ( $action == 'epkb-search-kb' ) {  // user searching KB
			...
			return;
		} else if ( in_array( $action, array( 'epkb_wpml_enable', 'epkb_preload_fonts','epkb_disable_openai', 'epkb_save_access_control', 'epkb_apply_settings_changes' ) ) ) {
