jQuery(document).ready(function($) {
	...
	// Link to open Main Page Tab instead of Frontend Editor when Modular Layout is active
	$( document ).on( 'click', '.epkb-admin__form .epkb-main-page-editor-link--modular-layout .epkb-cta__footer__button', function( e ) {
		e.preventDefault();
		$( '.epkb-admin__form .epkb-admin__form-tab[data-target="main-page"]' ).click();
		return false;
	});

	// Link to open Search Module settings
	$( document ).on( 'click', '.epkb-admin__form .epkb-admin__form-tab-content--search-box-settings .epkb-admin__form-tab-content-desc__link', function( e ) {
		e.preventDefault();
		$( '.epkb-admin__form .epkb-admin__form-tab[data-target="main-page"]' ).click();

		// Scroll to Row where the Search Module is selected or do nothing if not selected in any Row
		$( '.epkb-row-module-setting select' ).each( function() {
			if ( $( this ).val() === 'search' ) {
				$( this ).closest( '.epkb-admin__form-tab-content' ).get( 0 ).scrollIntoView();
				return false;
			}
		} );
	...
	// Link to open Search Box settings from Search Module settings
	$( document ).on( 'click', '.epkb-admin__form .eckb-ml-module__search .epkb-admin__form-tab-content-desc__link', function( e ) {
		e.preventDefault();
		$( '.epkb-admin__form .epkb-admin__form-tab[data-target="search-box"]' ).click();
		$( [document.documentElement, document.body] ).animate( {
			scrollTop: 0
		}, 300 );
		return false;
	...
	$( document ).on( 'click', '.eckb-conditional-setting-input', function() {
		...
		// Find current settings box container
		let current_settings_box = $( this ).closest( '.epkb-admin__form-tab-content' );

		// Find content that is dependent to the current input
		let dependent_targets = $( current_settings_box ).data( 'settings-group' )
			? $( current_settings_box ).find( '.eckb-condition-depend__' + current_input.attr( 'name' ) )
			: $( '.eckb-condition-depend__' + current_input.attr( 'name' ) );
		...
		dependent_targets.each( function() {
			...
			for ( let i = 0; i < all_dependency_fields.length; i++ ) {
				...
				// Find dependency field (limit dependency only to the current settings group if defined)
				let dependency_field = $( current_settings_box ).data( 'settings-group' )
					? $( current_settings_box ).find( '#' + all_dependency_fields[i] )
					: $( '#' + all_dependency_fields[i] );
				...
				if ( all_dependency_values.indexOf( dependency_input.val() ) >= 0 ) {
					...
					// Trigger click event of the current dependent field to check its own dependent fields (limit dependency only to the current settings group if defined)
					setTimeout( function() {
						if ( $( current_settings_box ).data( 'settings-group' ) ) {
							$( current_settings_box ).find( '#' + target_id ).trigger( 'click' );
						} else {
	...
	$( document ).on( 'click', '#epkb-admin-page-reload-confirmation .epkb-dbf__footer__accept__btn', function() {
		...
		// Do nothing if the confirmation dialog is called not for Main Page layout
		if ( ! $confirmation_dialog.hasClass( 'epkb-kb-main-page-layout--active' ) ) {
			return;
		}
		...
		$confirmation_dialog.removeClass( 'epkb-dialog-box-form--active' );
	...
	for( let i = 0; i < ml_module_names.length; i++ ) {
		$( document ).on( 'change', '[data-settings-group="ml-row"] .epkb-row-module-setting select', function() {
			...
			let current_input_name = $( this ).attr( 'name' );

			// Add module to current row
			let target_fields = $( '.eckb-ml-module__' + current_module_name );
			$( this ).closest( '.epkb-admin__kb__form' ).append( target_fields );
		} );
	}
	...
	$( document ).on( 'click', '.epkb-input-custom-dropdown__option', function() {
		...
		// Unset current value in other dropdowns of the current selection group
		let current_input_name = $( input_container ).find( 'select' ).attr( 'name' );
		let current_selection_group = $( this ).closest( '[data-custom-selection-group]' ).data( 'custom-selection-group' );
		$( '[data-custom-selection-group="' + current_selection_group + '"] select' ).each( function() {
	...
	// Module Categories & Articles: selection of Sidebar Positions
	$( document ).on( 'change', '.epkb-ml-categories-articles-sidebar-positions select', function() {
	...
	// Toggle collapse for admin form tabs
	$( document ).on( 'click', '.epkb-admin__form-tab-content--collapsable .epkb-admin__form-tab-content-title', function() {

		let current_box = $( this ).parent();

		// Do nothing for already expanded box
		if ( ! current_box.hasClass( 'epkb-admin__form-tab-content--collapsed' ) ) {
			return;
		}

		let collapse_group = current_box.data( 'collapse-setting-group' );

		// Collapse all boxes for the current group
		$( '[data-collapse-setting-group="' + collapse_group + '"]' ).addClass( 'epkb-admin__form-tab-content--collapsed' );

		// Expand current box
		current_box.removeClass( 'epkb-admin__form-tab-content--collapsed' );
	} );
});