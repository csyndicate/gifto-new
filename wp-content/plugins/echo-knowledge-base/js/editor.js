...
jQuery(document).ready(function($) {
	...
	window.EPKBEditor = {
		...
		afterLoadIframe: function( event ) {
			...
			app.iframe.on( 'click', 'body .epkb-ml__module', function( event ) {
				event.stopPropagation();
		...
		onMenuItemClick: function ( event ) {
			...
			if ( $(this).data('name') == 'layouts' ) {
				...
				// Show message for Modular Main Page settings if it is currently active
				if ( epkb_editor_config.settings_zone.settings.kb_main_page_layout.value === 'Modular' ) {
					epkb_editor_config.settings_zone.settings.kb_main_page_layout.values_info.forEach( function( valueInfo ) {
						if ( valueInfo.value === 'Modular' ) {
