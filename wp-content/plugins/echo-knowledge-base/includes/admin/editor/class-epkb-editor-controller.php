...
class EPKB_Editor_Controller {
	...
	private static function copy_settings_from_main( $new_config, $apply_common_config ) {
		...
		$relation = array(
			...
			'article_search_input_typography'            => 'search_input_typography',
	...
	public static function reset_sidebar_widths( $new_config ) {
		...
		if ( $is_left_sidebar_on ) {
			...
			$left_sidebar_tablet_width = $new_config['article-left-sidebar-tablet-width-v2'] ? $new_config['article-left-sidebar-tablet-width-v2'] : 20;
		}
		...
		if ( $is_right_sidebar_on ) {
			...
			$right_sidebar_tablet_width = $new_config['article-right-sidebar-tablet-width-v2'] ? $new_config['article-right-sidebar-tablet-width-v2'] : 20;
		}
	...
	public static function get_sidear_component_priority() {
	...
	private function update_category_icons( $new_config, $chosen_preset ) {
		...
		$current_category_icons = EPKB_KB_Config_Category::get_category_data_option( $new_config['id'] );
		...
		if ( $icons_updated ) {
			EPKB_Utilities::save_kb_option( $new_config['id'], EPKB_Icons::CATEGORIES_ICONS, $current_category_icons );
		}
	}
}