...
class EPKB_KB_Wizard_Cntrl {
	...
	public function apply_wizard_changes() {
		...
		switch( $wizard_type ) {
			case 'setup':
				...
				$wizard_fields = apply_filters( 'epkb_kb_theme_fields_list', $preset_fields );
				break;
			case 'ordering':
	...
	private function apply_setup_wizard_changes( $wizard_kb_id, $orig_config, $new_config ) {
		...
		if ( EPKB_Icons::is_theme_with_image_icons( $new_config ) ) {
			...
			$categories_data = EPKB_KB_Config_Category::get_category_data_option( $wizard_kb_id );
			$categories_data_ids = array();
			foreach( $categories_data as $term_id => $categories_icon ) {
				$categories_data_ids[] = $term_id;
			}
			...
			foreach ( $kb_categories as $kb_category ) {
				...
				if ( in_array( $term_id, $categories_data_ids ) ) {
					$categories_data[$term_id]['type'] = 'image';
					$categories_data[$term_id]['image_thumbnail_url'] = empty($categories_data[$term_id]['image_thumbnail_url']) ? Echo_Knowledge_Base::$plugin_url . EPKB_Icons::DEFAULT_IMAGE_SLUG: $categories_data[$term_id]['image_thumbnail_url'];
				} else {
					$image_icon = array(
						'type' => 'image',
					...
					$categories_data[$term_id] = $image_icon;
				}
			}

			EPKB_Utilities::save_kb_option( $wizard_kb_id, EPKB_Icons::CATEGORIES_ICONS, $categories_data );
		}
