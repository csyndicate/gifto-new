...
class EPKB_Export_Import {
	...
	public function import_kb_config( $kb_id ) {
		...
		foreach ($this->add_ons_info as $add_on_class => $add_on_prefix) {
			...
			if ( $add_on_prefix == 'epkb' && in_array( $add_on_config['kb_main_page_layout'], [ EPKB_KB_Config_Specs::GRID_LAYOUT, EPKB_KB_Config_Specs::SIDEBAR_LAYOUT ] ) && ! EPKB_Utilities::is_elegant_layouts_enabled() ) {
