	...
	private static function get_other_boxes( $kb_config ) {
		...
		$boxes_config[] = array(
			...
			'html'  => EPKB_HTML_Elements::checkbox_toggle( [
				...
				'textLoc'       => 'left',
				'checked'       => EPKB_Core_Utilities::is_kb_flag( 'disable_openai' ),
	...
	public static function display_debug_data() {
		...
		foreach ( $all_kb_ids as $kb_id ) {
			...
			foreach( $kb_config as $name => $value ) {
				...
				if ( ! isset( $_POST['epkb_show_full_debug'] ) && ! in_array($name, array('id','first_plugin_version', 'upgrade_plugin_version', 'modular_main_page_toggle', 'kb_main_pages','kb_name','kb_articles_common_path','article-structure-version','categories_in_url_enabled',
