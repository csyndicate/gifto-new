...
class EPKB_KB_Handler {
	...
	public static function get_current_kb_id() {
		...
		/*if ( ! headers_sent() && ( $kb_id != EPKB_KB_Config_DB::DEFAULT_KB_ID || isset( $_COOKIE['eckb_kb_id'] ) ) ) {
			setcookie( 'eckb_kb_id', $kb_id );
		}*/
	...
	private static function find_current_kb_id() {
		...
		// use KB ID from Cookies - only if user is on admin pages and the cookie is not empty
		$eckb_cookie_kb_id = isset( $_COOKIE['eckb_kb_id'] ) ? $_COOKIE['eckb_kb_id'] : '';
		if ( is_admin() && EPKB_Utilities::is_positive_int( $eckb_cookie_kb_id ) ) {
			return $eckb_cookie_kb_id;
		}
