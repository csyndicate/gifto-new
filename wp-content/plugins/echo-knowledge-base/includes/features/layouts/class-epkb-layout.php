...
abstract class EPKB_Layout {
	...
	public function show_categories_missing_message() {
		...
		// enqueue custom CSS file here that normally never used if KB has content
		$suffix = ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ) ? '' : '.min';
		wp_register_style( 'epkb-mp-custom-no-content', Echo_Knowledge_Base::$plugin_url . 'css/mp-custom-no-content' . $suffix . '.css', array(), Echo_Knowledge_Base::$version );
		wp_print_styles( array( 'epkb-mp-custom-no-content' ) );
		...
		<section class="eckb-kb-no-content">   <?php
			...
			if ( EPKB_Utilities::is_wpml_enabled( $this->kb_config ) && !empty( $_POST['action'] ) && $_POST['action'] == 'epkb_wizard_update_order_view' && current_user_can( EPKB_Admin_UI_Access::get_author_capability() ) ) { ?>
				<h2 class="eckb-kb-no-content-title"><?php esc_html_e( 'No KB categories found. This ordering action must be performed for each language separately. ' .
						'Only one language can be selected for this page in the top admin menu.', 'echo-knowledge-base' ); ?></h2>  <?php
