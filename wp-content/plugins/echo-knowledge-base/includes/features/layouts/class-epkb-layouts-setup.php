...
class EPKB_Layouts_Setup {
	...
	public static function get_kb_page_output_hook( $content, $the_content_filter_call = true ) {
		...
		if ( $kb_config['kb_article_page_layout'] == EPKB_KB_Config_Specs::SIDEBAR_LAYOUT && ! EPKB_Articles_Setup::is_article_structure_v2( $kb_config ) ) {
	...
	public static function output_main_page( $kb_config, $is_builder_on=false, $article_seq=array(), $categories_seq=array() ) {
		...
		$layout_output = '';
		if ( self::is_elay_layout( $layout ) && EPKB_Utilities::is_elegant_layouts_enabled() ) {

			if ( $layout == EPKB_KB_Config_Specs::SIDEBAR_LAYOUT && EPKB_Articles_Setup::is_article_structure_v2( $kb_config ) ) {
			...
			$layout = empty($layout_output) ? EPKB_Layout::BASIC_LAYOUT : $layout;
		}
	...
	public static function is_elay_layout( $layout ) {
		return $layout == EPKB_KB_Config_Specs::GRID_LAYOUT || $layout == EPKB_KB_Config_Specs::SIDEBAR_LAYOUT;
	}
