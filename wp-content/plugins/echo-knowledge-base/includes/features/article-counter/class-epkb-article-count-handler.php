...
class EPKB_Article_Count_Handler {
	...
	public static function is_article_recently_viewed( $article_id ) {
		...
		// check is cookie isset to track articles and their last view time
		if ( ! isset( $_COOKIE['epkb_article_views_counter'] ) ) {
			setcookie( 'epkb_article_views_counter', '', time() + MONTH_IN_SECONDS, '/' );
			return false;
		}

		$article_views = maybe_unserialize( $_COOKIE['epkb_article_views_counter'] );
	...
	public static function update_article_view_time_cookie( $article_id ) {
		...
		setcookie( 'epkb_article_views_counter', serialize( $article_views ), time() + MONTH_IN_SECONDS, '/' );
	}
