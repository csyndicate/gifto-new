	...
	*/
	...
	public static function get_article_content_and_features( $article, $content, $kb_config ) {
	   ...
	   $v2_suffix = $kb_config['article_content_enable_rows'] == 'on' ? '-v2' : '';
		...
		<style id="eckb-article-print-styles"><?php
			ob_start(); ?>
			@media print {
				@page {
	...
	public static function search_box( $args ) {
		...
		// use Module Search if Modular Layout is active
		if ( $args['config']['kb_main_page_layout'] == EPKB_KB_Config_Layout_Modular::LAYOUT_NAME ) {
			EPKB_Layout_Modular::search_module( $args['config'] );
			return;
		}
	...
	public static function display_nav_sidebar_left( $args ) {
		...
		if ( $nav_sidebar_type == 'eckb-nav-sidebar-categories' ) {
			self::display_categories_sidebar( $args );
	 ...
	 * @param $args
	 */
	private static function display_categories_sidebar($args ) {
	...
	private static function generate_article_structure_css_v2($kb_config ) {
		...
		<style><?php
			...
			@media only screen and ( max-width: <?php echo $mobile_breakpoint; ?>px ) {
				...
				#eckb-article-page-container-v2 #eckb-article-left-sidebar {
					order: 3;
					margin-top: <?php echo $article_left_sidebar_starting_position_mobile; ?>px!important;
				}
				#eckb-article-page-container-v2 #eckb-article-content { order: 2; }
				#eckb-article-page-container-v2 #eckb-article-right-sidebar {
					order: 1;
					margin-top: <?php echo $article_right_sidebar_starting_position_mobile; ?>px!important;
				}
			}   <?php
			$inline_styles = ob_get_clean();
			echo EPKB_Utilities::minify_css( $inline_styles );
	...
	public static function is_article_structure_v2( $kb_config ) {
		// deprecated
		return $kb_config['article-structure-version']  == 'version-2' || $kb_config['kb_main_page_layout'] == EPKB_Layout::CATEGORIES_LAYOUT;
	}
