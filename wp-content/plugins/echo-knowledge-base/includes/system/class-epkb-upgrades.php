...
class EPKB_Upgrades {
	...
	private static function upgrade_to_v730( &$kb_config ) {
		...
		if ( $old_image_found ) {
			EPKB_Utilities::save_kb_option( $kb_config['id'], EPKB_Icons::CATEGORIES_ICONS, $categories_icons );
		}
	}
	...
	private static function upgrade_to_v700( &$kb_config ) {
		...
		$is_v2_version = $kb_config['article-structure-version']  == 'version-2' || $kb_config['kb_main_page_layout'] == EPKB_Layout::CATEGORIES_LAYOUT;
		$left_sidebar =     $kb_config['kb_main_page_layout'] == EPKB_KB_Config_Specs::SIDEBAR_LAYOUT ||
		                    ...
		                    ( $kb_config['kb_main_page_layout'] == EPKB_Layout::CATEGORIES_LAYOUT && ! empty( $sidebar_priority['categories_left'] ) ); // Categories Layout always on the left */
		...
		$right_sidebar =    ( $is_v2_version && $sidebar_priority['toc_right'] ) ||                                                       // TOC v2
                            ...
                            ( $kb_config['kb_main_page_layout'] == EPKB_Layout::CATEGORIES_LAYOUT && ! empty( $sidebar_priority['categories_right'] ) );              // Categories Layout always on the right
