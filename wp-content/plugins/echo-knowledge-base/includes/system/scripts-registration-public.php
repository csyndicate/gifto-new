...
function epkb_load_public_resources() {
	...
	// CASE: KB Main Page
	} else if ( EPKB_Core_Utilities::is_kb_main_page() ) {
		...
		if ( isset( $_GET[$search_query_param] ) ) {
			...
			switch ( $kb_config['kb_main_page_layout'] ) {
				case 'Tabs': $current_css_file_slug = 'mp-frontend-tab-layout'; break;
				case 'Categories': $current_css_file_slug = 'mp-frontend-category-layout'; break;
				case 'Grid': $current_css_file_slug = EPKB_Utilities::is_elegant_layouts_enabled() ? 'mp-frontend-grid-layout' : 'mp-frontend-basic-layout'; break;
				case 'Sidebar': $current_css_file_slug = EPKB_Utilities::is_elegant_layouts_enabled() ? 'mp-frontend-sidebar-layout' : 'mp-frontend-basic-layout'; break;
				case 'Modular': $current_css_file_slug = 'mp-frontend-modular-layout'; break;
				case 'Basic':
				default: $current_css_file_slug = 'mp-frontend-basic-layout'; break;
			}
		}
	...
	// CASE: KB Article
		...
		$kb_config = epkb_get_instance()->kb_config_obj->get_kb_config_or_default( $eckb_kb_id );
		$current_css_file_slug = $kb_config['kb_main_page_layout'] == EPKB_KB_Config_Layout_Modular::LAYOUT_NAME
			? 'ap-frontend-modular-layout'
			: 'ap-frontend-layout';
...
function epkb_enqueue_public_resources( $kb_id=0 ) {
	...
	// enqueue only slug that was registered earlier
...
function epkb_frontend_kb_theme_styles_now( $kb_config, $css_file_slug ) {
	...
	$is_kb_main_page = in_array( $css_file_slug, [ 'mp-frontend-basic-layout', 'mp-frontend-tab-layout', 'mp-frontend-category-layout', 'mp-frontend-modular-layout', 'mp-frontend-grid-layout', 'mp-frontend-sidebar-layout' ] );
	...
	if ( in_array( $css_file_slug, [ 'mp-frontend-basic-layout', 'mp-frontend-tab-layout', 'mp-frontend-category-layout', 'mp-frontend-modular-layout' ] ) ) {
		...
		if ( $css_file_slug == 'mp-frontend-modular-layout' ) {
			$output .= EPKB_Layout_Modular::get_inline_styles( $kb_config );
		}
	}

	if ( in_array( $css_file_slug, [ 'mp-frontend-sidebar-layout', 'ap-frontend-layout', 'ap-frontend-modular-layout' ] ) ) {
		$output .= '
		...
		if ( $css_file_slug == 'ap-frontend-modular-layout' ) {
			$output .= EPKB_Layout_Modular::get_inline_styles( $kb_config, true );
		}
	}
...
function epkb_preload_fonts() {
	...
	if ( ! EPKB_Core_Utilities::is_kb_flag( 'preload_fonts' ) ) {
		return;
	}
