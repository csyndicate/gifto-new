...
class EPKB_HTML_Elements {
		...
		<div class="epkb-input-group epkb-admin__text-field <?php echo esc_html( $args['input_group_class'] ); ?>" id="<?php echo esc_attr( $args['name'] ); ?>_group" <?php echo $group_data_escaped; ?>>
			...
			<label class="<?php echo esc_attr( $args['label_class'] ); ?>" for="<?php echo esc_attr( $args['name'] ); ?>">  <?php
			    ...
			    if ( ! empty( $args['tooltip_body'] ) ) {
			        self::display_tooltip( $args['tooltip_title'], $args['tooltip_body'], $args['tooltip_args'] );
			    }
				if ( $args['is_pro'] ) {
	...
	public static function checkbox_toggle( $args = array() ) {
		$defaults = array(
			...
			'toggleOffext'  => __( 'off', 'echo-knowledge-base' ),
			'return_html'   => false,
		);
		$args       = self::add_defaults( $args, $defaults );
		$args       = self::get_specs_info( $args );
		...
		<div id="<?php echo esc_attr( $args['id'] ); ?>" class="epkb-settings-control-container epkb-settings-control-type-toggle <?php echo 'epkb-settings-control-type-toggle--' . esc_attr( $args['textLoc'] ); ?> <?php echo esc_attr( $args['input_group_class'] ); ?>" data-field="<?php echo esc_attr( $args['data'] ); ?>" <?php echo $group_data_escaped; ?>>     <?php
	...
	public static function custom_dropdown( $args = array() ) {
		...
		<div class="epkb-input-group epkb-input-custom-dropdown <?php echo esc_attr( $args['input_group_class'] ); ?>" id="<?php echo esc_attr( $args['name'] ); ?>_group" <?php echo $group_data_escaped; ?>>
			...
			<div class="input_container <?php echo esc_attr( $args['input_class'] ); ?>">
				...
				<div class="epkb-input-custom-dropdown__input"><span><?php echo esc_html( $args['options'][$args['value']] ); ?></span><i class="epkbfa epkbfa-chevron-down"></i></div>
	...
	public static function radio_buttons_horizontal( $args = array() ) {
        ...
        <div class="epkb-input-group epkb-radio-horizontal-button-group-container <?php echo esc_attr( $args['input_group_class'] ); ?>" id="<?php echo esc_attr( $args['name'] ); ?>_group" <?php echo $group_data_escaped; ?>>
	...
	private static function get_specs_info( $args ) {
		...
		if ( empty( $args['options'] ) && $args_specs['type'] == 'select' ) {
