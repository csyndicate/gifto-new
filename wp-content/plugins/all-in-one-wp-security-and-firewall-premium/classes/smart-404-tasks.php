...
class AIOWPS_Smart404_Tasks extends AIOWPS_Premium_Base_Tasks {
    ...
    public function do_smart_404_general_tasks() {
        ...
        if (is_404() && !is_user_logged_in()) {
            ...
            if ($aio_wp_security_premium->configs->get_value('aiowps_enable_instant_404_string_block') == '1') {
    ...
    /**
     * Uses the "filter_event_logger_data" filter which fires just before event log is saved to DB
     * Will find the country code of the IP address and add it to the data array
     *
	 * @param $data
     * @return $data array
     */
    ...
    public function get_404_count($ip_address) {
        global $wpdb, $aio_wp_security_premium, $aio_wp_security;
        $retry_interval_404 = $aio_wp_security_premium->configs->get_value('aiowps_404_retry_time_period');
        $now = current_time('mysql');
        $now_date_time = new DateTime($now);

        $sql = $wpdb->prepare('SELECT COUNT(ID) FROM ' . AIOWPSEC_TBL_EVENTS .
                ' WHERE event_date + INTERVAL ' . $retry_interval_404 .
                ' MINUTE > "' . $now_date_time->format('Y-m-d H:i:s') . '" AND event_type=%s AND ' .
                'ip_or_host=%s', '404', $ip_address);
