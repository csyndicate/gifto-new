...
if ( ! function_exists( 'woosb_update_checker' ) ) {
	...
	add_action( 'in_plugin_update_message-' . plugin_basename( WOOSB_FILE ), 'woosb_update_license', 99 );
	...
	function woosb_update_checker() {
		if ( $key = wpc_get_update_key( 5028 ) ) {
			...
			PucFactory::buildUpdateChecker( 'https://api.wpclever.net/update/5028.json', WOOSB_FILE, plugin_basename( WOOSB_DIR ) );
		}
	}

	function woosb_update_license( $plugin_data ) {
		if ( empty( $plugin_data['package'] ) ) {
			echo ' Please verify your license key in <a href="' . admin_url( 'admin.php?page=wpclever-keys' ) . '" target="_blank">WPClever > License Keys</a> to update.';
		}
	}
		...
		if ( ! wpc_get_update_key( 5028 ) ) {
            ...
            <div data-dismissible="woosb_update" class="wpc-notice notice notice-warning is-dismissible">
                <p>Please verify <a href="<?php echo admin_url( 'admin.php?page=wpclever-keys' ); ?>">License Key</a> of
                    <strong>WPC Product Bundles</strong> to enjoy unlimited update release and get the latest plugin update directly on the website backend.
                </p>
            </div>
			<?php
		}
	...
	class WPCleverPremium {
		...
		function admin_enqueue_scripts( $hook ) {
			if ( strpos( $hook, 'wpclever-keys' ) ) {
				wp_enqueue_style( 'wpc-premium', WOOSB_URI . 'includes/premium/css/premium.css' );
				wp_enqueue_script( 'wpc-premium', WOOSB_URI . 'includes/premium/js/premium.js', [ 'jquery', ] );
			}
		}
