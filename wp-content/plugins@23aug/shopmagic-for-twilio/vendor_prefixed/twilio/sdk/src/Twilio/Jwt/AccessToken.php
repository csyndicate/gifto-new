...
use ShopMagicTwilioVendor\Twilio\Jwt\Grants\Grant;
class AccessToken
{
     ...
     */
    public function addGrant(\ShopMagicTwilioVendor\Twilio\Jwt\Grants\Grant $grant)
    {
        $this->grants[] = $grant;
        return $this;
    }
    ...
    public function toJWT($algorithm = 'HS256')
    {
        ...
        $payload = \array_merge($this->customClaims, array('jti' => $this->signingKeySid . '-' . $now, 'iss' => $this->signingKeySid, 'sub' => $this->accountSid, 'exp' => $now + $this->ttl, 'grants' => $grants));
        if (!\is_null($this->nbf)) {
            $payload['nbf'] = $this->nbf;
        }
        return \ShopMagicTwilioVendor\Twilio\Jwt\JWT::encode($payload, $this->secret, $algorithm, $header);
    }
