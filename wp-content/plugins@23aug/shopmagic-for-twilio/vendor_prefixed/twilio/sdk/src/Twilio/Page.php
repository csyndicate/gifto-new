...
{
    protected static $metaKeys = array('end', 'first_page_uri', 'next_page_uri', 'last_page_uri', 'page', 'page_size', 'previous_page_uri', 'total', 'num_pages', 'start', 'uri');
    protected $version;
    protected $payload;
    protected $solution;
    protected $records;
    ...
    protected function processResponse(\ShopMagicTwilioVendor\Twilio\Http\Response $response)
    {
        if ($response->getStatusCode() != 200 && !$this->isPagingEol($response->getContent())) {
            ...
            throw new \ShopMagicTwilioVendor\Twilio\Exceptions\RestException($message, $code, $response->getStatusCode());
        }
