...
namespace ShopMagicTwilioVendor\Twilio\Jwt;

use ShopMagicTwilioVendor\Twilio\Jwt\Client\ScopeURI;
/**
...
class ClientToken
{
    ...
    {
        $this->allow('client', 'outgoing', array('appSid' => $appSid, 'appParams' => \http_build_query($appParams, '', '&')));
    }
    /**
    ...
    {
        $this->allow('stream', 'subscribe', array('path' => '/2010-04-01/Events', 'params' => \http_build_query($filters, '', '&')));
    }
    /**
    ...
    {
        $payload = \array_merge($this->customClaims, array('scope' => array(), 'iss' => $this->accountSid, 'exp' => \time() + $ttl));
        $scopeStrings = array();
    ...
    {
        $this->scopes[] = new \ShopMagicTwilioVendor\Twilio\Jwt\Client\ScopeURI($service, $privilege, $params);
    }
}
