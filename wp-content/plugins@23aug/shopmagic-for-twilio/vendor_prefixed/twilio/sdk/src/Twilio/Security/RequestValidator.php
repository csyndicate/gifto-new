...
namespace ShopMagicTwilioVendor\Twilio\Security;
...
final class RequestValidator
{
    ...
    public function computeSignature($url, $data = array())
    {
        ...
        return \base64_encode(\hash_hmac('sha1', $url, $this->authToken, \true));
    }
    /**
    ...
    {
        return \bin2hex(\hash('sha256', $data, \true));
    }
    /**
    ...
    public function validate($expectedSignature, $url, $data = array())
    {
        ...
        $urlWithPort = \ShopMagicTwilioVendor\Twilio\Security\RequestValidator::addPort($parsedUrl);
        $urlWithoutPort = \ShopMagicTwilioVendor\Twilio\Security\RequestValidator::removePort($parsedUrl);
        $validBodyHash = \true;
        // May not receive body hash, so default succeed
        if (!\is_array($data)) {
            // handling if the data was passed through as a string instead of an array of params
            $queryString = \explode('?', $url);
            $queryString = $queryString[1];
            \parse_str($queryString, $params);
            $validBodyHash = self::compare(self::computeBodyHash($data), \ShopMagicTwilioVendor\Twilio\Values::array_get($params, 'bodySHA256'));
            $data = array();
        }
        /*
    ...
    private static function removePort($parsedUrl)
    {
        ...
        return \ShopMagicTwilioVendor\Twilio\Security\RequestValidator::buildUrl($parsedUrl);
    }
    /*
