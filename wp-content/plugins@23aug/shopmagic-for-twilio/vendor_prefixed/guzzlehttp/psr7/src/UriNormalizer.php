...
final class UriNormalizer
{
    ...
    public static function normalize(\Psr\Http\Message\UriInterface $uri, int $flags = self::PRESERVING_NORMALIZATIONS) : \Psr\Http\Message\UriInterface
    {
        ...
        if ($flags & self::REMOVE_DEFAULT_PORT && $uri->getPort() !== null && \ShopMagicTwilioVendor\GuzzleHttp\Psr7\Uri::isDefaultPort($uri)) {
            $uri = $uri->withPort(null);
        }
        if ($flags & self::REMOVE_DOT_SEGMENTS && !\ShopMagicTwilioVendor\GuzzleHttp\Psr7\Uri::isRelativePathReference($uri)) {
            $uri = $uri->withPath(\ShopMagicTwilioVendor\GuzzleHttp\Psr7\UriResolver::removeDotSegments($uri->getPath()));
        }
    ...
    private static function capitalizePercentEncoding(\Psr\Http\Message\UriInterface $uri) : \Psr\Http\Message\UriInterface
    {
        $regex = '/(?:%[A-Fa-f0-9]{2})++/';
    ...
    private static function decodeUnreservedCharacters(\Psr\Http\Message\UriInterface $uri) : \Psr\Http\Message\UriInterface
    {
