...
class UploadedFile implements \Psr\Http\Message\UploadedFileInterface
{
    ...
    private function setStreamOrFile($streamOrFile) : void
    {
        if (\is_string($streamOrFile)) {
            ...
            $this->stream = new \ShopMagicTwilioVendor\GuzzleHttp\Psr7\Stream($streamOrFile);
    ...
    {
        if (\false === \in_array($error, \ShopMagicTwilioVendor\GuzzleHttp\Psr7\UploadedFile::ERRORS, \true)) {
    ...
    public function getStream() : \Psr\Http\Message\StreamInterface
    {
        ...
        /** @var string $file */
        $file = $this->file;
        return new \ShopMagicTwilioVendor\GuzzleHttp\Psr7\LazyOpenStream($file, 'r+');
    }
    public function moveTo($targetPath) : void
    {
        ...
        if ($this->file) {
            ...
            \ShopMagicTwilioVendor\GuzzleHttp\Psr7\Utils::copyToStream($this->getStream(), new \ShopMagicTwilioVendor\GuzzleHttp\Psr7\LazyOpenStream($targetPath, 'w'));
            $this->moved = \true;
        }
        if (\false === $this->moved) {
