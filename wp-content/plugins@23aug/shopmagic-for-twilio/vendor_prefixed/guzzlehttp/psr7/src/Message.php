...
final class Message
{
    ...
    public static function parseMessage(string $message) : array
    {
        ...
        if (\preg_match("/(?:^HTTP\\/|^[A-Z]+ \\S+ HTTP\\/)(\\d+(?:\\.\\d+)?)/i", $startLine, $matches) && $matches[1] === '1.0') {
            ...
            $rawHeaders = \preg_replace(\ShopMagicTwilioVendor\GuzzleHttp\Psr7\Rfc7230::HEADER_FOLD_REGEX, ' ', $rawHeaders);
        }
        /** @var array[] $headerLines */
        $count = \preg_match_all(\ShopMagicTwilioVendor\GuzzleHttp\Psr7\Rfc7230::HEADER_REGEX, $rawHeaders, $headerLines, \PREG_SET_ORDER);
        // If these aren't the same, then one line didn't match and there's an invalid header.
        if ($count !== \substr_count($rawHeaders, "\n")) {
            // Folding is deprecated, see https://tools.ietf.org/html/rfc7230#section-3.2.4
            if (\preg_match(\ShopMagicTwilioVendor\GuzzleHttp\Psr7\Rfc7230::HEADER_FOLD_REGEX, $rawHeaders)) {
    ...
    public static function parseRequest(string $message) : \Psr\Http\Message\RequestInterface
    {
        ...
        $request = new \ShopMagicTwilioVendor\GuzzleHttp\Psr7\Request($parts[0], $matches[1] === '/' ? self::parseRequestUri($parts[1], $data['headers']) : $parts[1], $data['headers'], $data['body'], $version);
    ...
    public static function parseResponse(string $message) : \Psr\Http\Message\ResponseInterface
    {
        ...
        $parts = \explode(' ', $data['start-line'], 3);
        return new \ShopMagicTwilioVendor\GuzzleHttp\Psr7\Response((int) $parts[1], $data['headers'], $data['body'], \explode('/', $parts[0])[1], $parts[2] ?? null);
    }
}
