...
final class Utils
{
    ...
    public static function streamFor($resource = '', array $options = []) : \Psr\Http\Message\StreamInterface
    {
        if (\is_scalar($resource)) {
            ...
            return new \ShopMagicTwilioVendor\GuzzleHttp\Psr7\Stream($stream, $options);
        }
        switch (\gettype($resource)) {
            case 'resource':
                ...
                return new \ShopMagicTwilioVendor\GuzzleHttp\Psr7\Stream($resource, $options);
            case 'object':
                /** @var object $resource */
                if ($resource instanceof \Psr\Http\Message\StreamInterface) {
                    return $resource;
                } elseif ($resource instanceof \Iterator) {
                    return new \ShopMagicTwilioVendor\GuzzleHttp\Psr7\PumpStream(function () use($resource) {
                        if (!$resource->valid()) {
                            return \false;
                        }
                ...
                break;
            case 'NULL':
                return new \ShopMagicTwilioVendor\GuzzleHttp\Psr7\Stream(self::tryFopen('php://temp', 'r+'), $options);
        }
        if (\is_callable($resource)) {
            return new \ShopMagicTwilioVendor\GuzzleHttp\Psr7\PumpStream($resource, $options);
        }
