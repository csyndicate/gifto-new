...
use ShopMagicTwilioVendor\GuzzleHttp\Psr7\Exception\MalformedUriException;
...
class Uri implements \Psr\Http\Message\UriInterface, \JsonSerializable
{
    ...
    public function __construct(string $uri = '')
    {
        if ($uri !== '') {
            ...
            if ($parts === \false) {
                throw new \ShopMagicTwilioVendor\GuzzleHttp\Psr7\Exception\MalformedUriException("Unable to parse URI: {$uri}");
            }
    ...
    {
        if ($base !== null) {
            $uri = \ShopMagicTwilioVendor\GuzzleHttp\Psr7\UriResolver::resolve($base, $uri);
     ...
     * @return string[]
     */
    private static function getFilteredQueryString(\Psr\Http\Message\UriInterface $uri, array $keys) : array
    {
    ...
    private function validateState() : void
    {
        ...
        if ($this->getAuthority() === '') {
            if (0 === \strpos($this->path, '//')) {
                throw new \ShopMagicTwilioVendor\GuzzleHttp\Psr7\Exception\MalformedUriException('The path of a URI without an authority must not start with two slashes "//"');
            }
            if ($this->scheme === '' && \false !== \strpos(\explode('/', $this->path, 2)[0], ':')) {
                throw new \ShopMagicTwilioVendor\GuzzleHttp\Psr7\Exception\MalformedUriException('A relative URI must not have a path beginning with a segment containing a colon');
            }
        }
    }
}
