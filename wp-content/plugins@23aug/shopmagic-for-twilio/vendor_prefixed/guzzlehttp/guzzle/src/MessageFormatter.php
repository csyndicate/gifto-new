 ...
 * @final
 */
class MessageFormatter implements \ShopMagicTwilioVendor\GuzzleHttp\MessageFormatterInterface
{
    /**
    ...
    public function format(\Psr\Http\Message\RequestInterface $request, ?\Psr\Http\Message\ResponseInterface $response = null, ?\Throwable $error = null) : string
    {
        ...
        return \preg_replace_callback('/{\\s*([A-Za-z_\\-\\.0-9]+)\\s*}/', function (array $matches) use($request, $response, $error, &$cache) {
            ...
            switch ($matches[1]) {
                case 'request':
                    $result = \ShopMagicTwilioVendor\GuzzleHttp\Psr7\Message::toString($request);
                    break;
                case 'response':
                    $result = $response ? \ShopMagicTwilioVendor\GuzzleHttp\Psr7\Message::toString($response) : '';
                    break;
                case 'req_headers':
     ...
     */
    private function headers(\Psr\Http\Message\MessageInterface $message) : string
    {
        $result = '';
