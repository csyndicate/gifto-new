<?php
/*
Plugin Name: WPC Smart Wishlist for WooCommerce (Premium)
		...
		if ( ! class_exists( 'WPCleverWoosw' ) ) {
			class WPCleverWoosw {
				...
				function __construct() {
					...
					add_action( 'wp_ajax_nopriv_add_note', array( $this, 'add_note' ) );

					// manage wishlists
					add_action( 'wp_ajax_manage_wishlists', array( $this, 'manage_wishlists' ) );
					add_action( 'wp_ajax_nopriv_manage_wishlists', array( $this, 'manage_wishlists' ) );

					// view wishlist
					add_action( 'wp_ajax_view_wishlist', array( $this, 'view_wishlist' ) );
					add_action( 'wp_ajax_nopriv_view_wishlist', array( $this, 'view_wishlist' ) );
					...
					add_action( 'wp_ajax_nopriv_delete_wishlist', array( $this, 'delete_wishlist' ) );

					// set default
					add_action( 'wp_ajax_set_default', array( $this, 'set_default' ) );
					add_action( 'wp_ajax_nopriv_set_default', array( $this, 'set_default' ) );
                    ...
                    <div class="wpclever_settings_page wrap">
                        ...
                        <div class="wpclever_settings_page_nav">
                            <h2 class="nav-tab-wrapper">
                                ...
                                -->
                                <a href="<?php echo esc_url( WOOSW_SUPPORT ); ?>" class="nav-tab" target="_blank">
									<?php esc_html_e( 'Support', 'woo-smart-wishlist' ); ?>
                                </a>
						...
						foreach ( $products as $product_id => $product_data ) {
                            ...
                            <tr class="woosw-item woosw-item-<?php echo esc_attr( $product_id ); ?>"
                                ...
                                <td class="woosw-item--info">
									...
									if ( self::can_edit( $key ) || ( get_option( 'woosw_show_note', 'no' ) === 'yes' ) ) { ?>
                                        <div class="woosw-item--note">
											<?php echo( empty( $product_note ) ? self::localization( 'add_note', esc_html__( 'Add note', 'woo-smart-wishlist' ) ) : nl2br( $product_note ) ); ?>
                                        </div>
                                        <div class="woosw-item--note-add" style="display: none">
                                            <input type="text" value="<?php echo esc_attr( $product_note ); ?>"/>
                                            <input type="button" class="woosw_add_note"
                                                   value="<?php echo self::localization( 'save_note', esc_attr__( '✓', 'woo-smart-wishlist' ) ); ?>"/>
                                        </div>
									<?php }
				...
				function wp_footer() {
					...
					if ( ( get_option( 'woosw_enable_multiple', 'no' ) === 'yes' ) && is_user_logged_in() ) {
						echo '<div id="woosw_manage" class="woosw-popup"></div>';
					}
				}
				...
				function wishlist_content( $key = false, $message = '' ) {
					...
					if ( ( $user_id = get_current_user_id() ) && ( get_option( 'woosw_enable_multiple', 'no' ) === 'yes' ) ) {
						$keys = get_user_meta( $user_id, 'woosw_keys', true ) ?: array();

						if ( isset( $keys[ $key ] ) ) {
							if ( isset( $keys[ $key ]['type'] ) && ( $keys[ $key ]['type'] === 'primary' ) ) {
								$name = self::localization( 'primary_name', esc_html__( 'Wishlist', 'woo-smart-wishlist' ) );
							} else {
								if ( isset( $keys[ $key ]['name'] ) && ! empty( $keys[ $key ]['name'] ) ) {
									$name = $keys[ $key ]['name'];
								} else {
									$name = $key;
								}
                    ...
                    <div class="woosw-popup-inner" data-key="<?php echo esc_attr( $key ); ?>">
                        <div class="woosw-popup-content">
                            <div class="woosw-popup-content-top">
								...
								if ( ( get_option( 'woosw_enable_multiple', 'no' ) === 'yes' ) && is_user_logged_in() ) {
									echo '<span class="woosw-manage">' . self::localization( 'manage_wishlists', esc_html__( 'Manage wishlists', 'woo-smart-wishlist' ) ) . '</span>';
								}
								?>
										...
										if ( is_array( $keys ) && ! empty( $keys ) ) {
											foreach ( $keys as $k => $wl ) {
												...
												if ( isset( $wl['type'] ) && ( $wl['type'] === 'primary' ) ) {
													echo '<a class="woosw-view-wishlist" href="' . self::get_url( $k, true ) . '" data-key="' . esc_attr( $k ) . '">' . self::localization( 'primary_name', esc_html__( 'Wishlist', 'woo-smart-wishlist' ) ) . '</a> - primary (' . $count . ')';
												} else {
													if ( ! empty( $wl['name'] ) ) {
														echo '<a class="woosw-view-wishlist" href="' . self::get_url( $k, true ) . '" data-key="' . esc_attr( $k ) . '">' . $wl['name'] . '</a> (' . $count . ')';
													} else {
														echo '<a class="woosw-view-wishlist" href="' . self::get_url( $k, true ) . '" data-key="' . esc_attr( $k ) . '">' . $k . '</a> (' . $count . ')';
													}
												}
												...
												if ( $key === $k ) {
													...
													echo '<a class="woosw-set-default" data-key="' . $k . '" href="#">' . self::localization( 'set_default', esc_html__( 'set default', 'woo-smart-wishlist' ) ) . '</a>';
												}

												echo '</td><td style="text-align: end">';

												if ( ( ! isset( $wl['type'] ) || ( $wl['type'] !== 'primary' ) ) && ( $key !== $k ) ) {
													echo '<a class="woosw-delete-wishlist" data-key="' . $k . '" href="#">' . self::localization( 'delete', esc_html__( 'delete', 'woo-smart-wishlist' ) ) . '</a>';
												}
