		...
		if(is_plugin_active('gravityforms/gravityforms.php')){
			...
			$script .= '(function($){
				jQuery(".gform_next_button").on("click", function (e) {
					$ccs_phone = jQuery("input#'.$field_id.'");
					jQuery("input[type=hidden][name="+$ccs_phone.attr("name")+"]").val(jQuery("input#'.$field_id.'").val());
					setCookie("phone_number'.$field_id.'",jQuery("input#'.$field_id.'").val(),1);
					jQuery("#gform_'.$form['id'].'").submit();
				});
			})(jQuery);';
			...
			$script .= '(function($){
				jQuery(".gform_previous_button").on("click", function (e) {
					$ccs_phone = jQuery("input#'.$field_id.'");
					var saved_number = getCookie("phone_number'.$field_id.'");
					if(saved_number!==""){
						jQuery("input#'.$field_id.'").val(saved_number);
						jQuery("input#'.$field_id.'").attr("val",saved_number);
					...
					iti.promise.then(function() {
						setTimeout(()=>{
							var saved_number = getCookie("phone_number'.$field_id.'");
							jQuery(".intl-tel-input input[name=input_'.get_option('gform_phone_field_id').'").val(saved_number);
						},100);
