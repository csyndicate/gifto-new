...
! defined( 'WOOST_KEY' ) && define( 'WOOST_KEY', 'p1z2t5O3w7' );

if ( ! function_exists( 'woost_update_checker' ) ) {
	add_action( 'init', 'woost_update_checker', 99 );
	function woost_update_checker() {
		if ( defined( 'WOOST_KEY' ) && wpc_allow_update( WOOST_KEY ) ) {
			PucFactory::buildUpdateChecker( 'https://api.wpclever.net/update/' . WOOST_KEY . '.json', WOOST_FILE, 'wpc-product-tabs-premium' );
		}
	}
}
...
if ( ! class_exists( 'WPCleverPremium' ) ) {
	class WPCleverPremium {
		...
		function admin_enqueue_scripts() {
			wp_enqueue_style( 'wpc-premium', WOOST_URI . 'assets/css/premium.css' );
			wp_enqueue_script( 'wpc-premium', WOOST_URI . 'assets/js/premium.js', array( 'jquery', ), null, true );
		}
