...
class Async
{
    ...
    public static function init()
    {
        ...
        if (defined('DOING_AJAX') && DOING_AJAX) {
            ...
            if (ApiSe::getInstance()->checkStartAsync()) {
                ...
                if (ApiSe::getInstance()->checkObjectAsyncEnabled()) {
                    if (is_admin()) {
                        add_action('admin_footer', function() use ($async_url) {
                            self::addAsyncObjects($async_url);
                        }, 100);
                    } else {
                        add_action('wp_footer', function() use ($async_url) {
                            self::addAsyncObjects($async_url);
                        }, 100);
                    }

                } elseif (ApiSe::getInstance()->checkAjaxAsyncEnabled()) {
                    self::addJQueryObjects($async_url);
                }
            }
        } //endif async
        ...
        if (!defined('DOING_AJAX') && !defined('DOING_CRON')) {
            if (is_admin()) {
                ApiSe::getInstance()->checkExportStatusIsDone();
            } else {
     ...
     */
    public static function addJQueryObjects($async_url)
    {
        wc_enqueue_js(
            "jQuery.ajax({
                method: 'get',
     ...
     * @param array $lang_codes_to_processed
     */
    public static function addAsyncObjects($async_url)
    {
        echo "<object data=\"{$async_url}?action=se_async\" width=\"0\" height=\"0\" type=\"text/html\"></object>";
    }
    ...
    public function async($lang_code = null, $fl_ignore_processing = false)
    {
        ...
        if (!empty($lang_code) && ApiSe::getInstance()->getLocale($lang_code) != $lang_code) {
        ...
        Profiler::startBlock('async');
        ApiSe::getInstance()->echoProgress('.');
        // Read first element from queue
        $q = Queue::getInstance()->getNextQueue(null, $lang_code);

        while(!empty($q)) {
            Logger::getInstance()->debug($q);
            ...
            if (empty($private_key)) {
                Queue::getInstance()->deleteQueueById($q->queue_id);
                $q = array();
                continue;
            }
            ...
            Queue::getInstance()->setQueueStatusProcessing($q->queue_id);

            try {
                Profiler::startBlock($q->action . ':' . $q->queue_id);

                if ($q->action == Queue::PREPARE_FULL_IMPORT) {
                    Queue::getInstance()->prepareFullImport($engine['lang_code']);
                    ...
                    $this->addTaskByChunk($engine['lang_code'], Queue::UPDATE_PRODUCTS, true);
                    Queue::getInstance()->insertData(array(
                        'data'      => Queue::NO_DATA,
                    ...
                    $this->addTaskByChunk($engine['lang_code'], Queue::UPDATE_CATEGORIES, true);
                    $this->addTaskByChunk($engine['lang_code'], Queue::UPDATE_PAGES, true);
                ...
                } elseif ($q->action == Queue::START_FULL_IMPORT) {
                ...
                } elseif ($q->action == Queue::GET_INFO) {
                    ...
                    if (!empty($info['use_navigation'])) {
                        ApiSe::getInstance()->setNavigationEnabled($info['use_navigation'], $engine['lang_code']);
                    }

                    if (!empty($info['is_weglot_enabled'])) {
                        ApiSe::getInstance()->setIntegrationWeglotEnabled($info['is_weglot_enabled'], $engine['lang_code']);
                    }

                    $status = true;

                } elseif ($q->action == Queue::END_FULL_IMPORT) {
                    ...
                    if (ApiSe::getInstance()->isResultWidgetEnabled($engine['lang_code'])) {
                        ...
                        Installer::deleteSearchResultsPage();
                    }
                    ...
                    switch($q->action) {
                        ...
                        case Queue::UPDATE_ATTRIBUTES:
                            ...
                            if (!empty($facets)) {
                                ...
                                $data_for_send = $this->getTranslate($facets_data, $engine['lang_code']);
                            } else {
                                $status = true;
                            }
                    ...
                    if (!empty($type)) {
                        foreach ($data as $item_id) {
                            ...
                            if ($q->action == Queue::DELETE_FACETS) {
                    ...
                    // Unknown action name
                    throw new SearchaniseException(__('Unknown queue action', 'woocommerce-searchanise'));
                } // End if

                // Check for database errors
                if ($this->wpdb->last_error != '') {
                    throw new SearchaniseException(__('SQL Error', 'woocommerce-searchanise') . ' ' . $this->wpdb->last_error . '. Query: ' . $this->wpdb->last_query);
                }

                Profiler::endBlock($q->action . ':' . $q->queue_id);
            ...
            Logger::getInstance()->debug(array('status' => $status));

            if ($status == true) {
                Queue::getInstance()->deleteQueueById($q->queue_id);
                $q = Queue::getInstance()->getNextQueue($q->queue_id, $lang_code);

            } else {
                $next_started_time = time() - ApiSe::getInstance()->getMaxProcessingTime() + $q->attempts * 60;
                Queue::getInstance()->setQueueErrorById($q->queue_id, $next_started_time, $error);
    ...
    public function getHeader($lang_code)
    {
        ...
        $header = array(
            'header' => array(
                'id'      => ApiSe::getInstance()->getLanguageLink($shop_url, $lang_code),
                'updated' => date('c'),
            ),
        );
    ...
    private function addTaskByChunk($lang_code, $action, $isOnlyActive = true)
    {
        ...
        do {
            ...
            foreach ($chunk_item_id as $item_ids) {
                ...
                Queue::getInstance()->insertData($queue_data);
    ...
    public function getAttributeFilters($lang_code, $position = 30)
    {
        ...
        $system_custom_attributtes = ApiSe::getInstance()->getCustomAttributes();
        $custom_attributes = $this->checkAttributtes($system_custom_attributtes);

        if (!empty($custom_attributes)) {
            foreach ($custom_attributes as $custom) {
                $taxonomy = get_taxonomy($custom);
                $filters[] = self::generateFilterFromAttribute($taxonomy, $lang_code, 'select', $position);
                $position += 5;
            }
        }
    ...
    {
        $filter = array(
            'name'        => self::getTaxonomyId(isset($attr->name) ? $attr->name : $attr->attribute_name),
            'label'       => __(isset($attr->label) ? $attr->label : $attr->attribute_label, 'woocommerce'),
            'text_search' => 'Y',
            'type'        => $type,
        );
    ...
    {
        $product_tags = [];
        $_product_tags = get_terms(array(
            'taxonomy'   => 'product_tag',
            'hide_empty' => false,
        ));

        foreach ($_product_tags as $tag_term) {
            $product_tags[] = $tag_term instanceof \WP_Term ? [
                'name' => $tag_term->slug,
                'label' => $tag_term->name,
            ] : $tag_term;
        }
    ...
    public function getProductsData($product_ids, $lang_code, $lf_echo = true)
    {
        ...
        return $this->getTranslate($products_data, $lang_code);
    }
     ...
     * @return boolean
     */
    public function generateUsergroupProductPrices(&$entry, $product_data, $lang_code, $children_products = array())
    {
        ...
        if ($this->isUsergroupPricesAvailable()) {
            foreach ($this->getUserGroups() as $role) {
                ...
                $entry[ApiSe::LABEL_FOR_MAX_PRICES_USERGROUP . $role] = array(
                    'value' => (float)$prices['max_price'],
                    'title' => __('Max price for ', 'woocommerce-searchanise') . $role,
                    'type'  => 'float',
                );

                $entry[ApiSe::LABEL_FOR_LIST_PRICES_USERGROUP . $role] = array(
                    'value' => (float)$prices['regular_price'],
    ...
    public function prepareProductData($product_data, $lang_code)
    {
        ...
        $entry = array(
            ...
            'link' => array(
                'value' => ApiSe::getInstance()->getLanguageLink($product_data->get_permalink(), $lang_code),
        ...
        $this->generateUsergroupProductPrices($entry, $product_data, $lang_code, isset($children) ? $children : array());
        ...
        $customAttributes = $this->checkAttributtes(ApiSe::getInstance()->getCustomAttributes());
        ...
        if (!empty($customAttributes)) {
            $this->generateCustomAttribute($entry, $customAttributes, $product_data, $lang_code);
        }

        $products_meta_fields = ApiSe::getInstance()->getCustomProductFields();

        if (!empty($products_meta_fields)) {
            $this->generateProductMetaFields($entry, $products_meta_fields, $product_data);
        }
        ...
        $entry['add_to_cart_url'] = array(
            ...
            'sorting'     => 'N',
            'filter_type' => 'none',
            'value'       => $this->getAddToCartProductUrl($product_data), $lang_code,
        );
    ...
    public function generateProductAttribute(&$entry, $attr, $lang_code)
    {
        if ($attr->is_taxonomy()) {
            ...
            $entry[self::getTaxonomyId($taxonomy_object->attribute_name)] = $attribute_data;
    ...
    public function getProductQuantity(\WC_Product $product, $united_products = array())
    {
        ...
        if ((get_option('woocommerce_manage_stock') == 'yes' && $product->get_manage_stock()) || !empty($united_products)) {
            ...
            if ($quantity <= 0) {
                $quantity = $this->getQuantity($product->get_stock_status());
            }

            if (!empty($united_products) && $quantity != 1) { // TODO: Check if really needed
                $quantities = [];

                foreach ($united_products as $_product) {
                    $quantities[] = $this->getProductQuantity($_product);
                }
    ...
    public function getCategoriesData($category_ids, $lang_code)
    {
        ...
        foreach ($categories as $cat) {
            ...
            $category_data = array(
                ...
                'link'          => ApiSe::getInstance()->getLanguageLink(get_term_link($cat), $lang_code),
                'title'         => $cat->name,
        ...
        return $this->getTranslate($categories_data, $lang_code);
    }
    ...
    public function getPagesData($page_ids, $lang_code)
    {
        ...
        foreach ($pages as $post) {
            if (
                ...
                $page_data = array(
                    'id'         => $post->ID,
                    'link'       => ApiSe::getInstance()->getLanguageLink(get_permalink($post), $lang_code),
                    'title'      => $post->post_title,
                    'summary'    => self::STRIP_POST_CONTENT ? $this->getStripPostContent($post) : $post->post_content,
        ...
        return $this->getTranslate($pages_data, $lang_code);
    }
     ...
     */
    public function getStripPostContent($post)
    {
        $excerpt = $this->removeContentNoise($post->post_content);
     ...
     * @return string
     */
    public function removeContentNoise($content)
    {
    ...
    {
        $excluded_pages = ApiSe::getInstance()->getSystemSetting('excluded_pages');
    ...
    {
        $excluded_categories = ApiSe::getInstance()->getSystemSetting('excluded_categories');
    ...
    public static function ajaxAsync()
    {
        ...
        $lang_code = !empty($_REQUEST[self::FL_LANG_CODE]) ? $_REQUEST[self::FL_LANG_CODE] : null;

        if ($lang_code && !ApiSe::getInstance()->checkPrivateKey($lang_code)) {
        ...
        $status = self::getInstance()->async($lang_code, $fl_ignore_processing);
     ...
     * @return array
     */
    public function generateCustomProductAttribute()
    {
     ...
     * @return void
     */
    public function generateCustomAttribute(&$entry, $attributes, $data, $lang_code)
    {
        foreach ($attributes as $custom) {
            ...
            $attribute_data = array(
                ...
                'text_search' => 'Y',
                'weight'      => 0,
                'value'       => $this->getOptions($terms, $lang_code),
            );

            $entry[self::getTaxonomyId($taxonomy->name)] = $attribute_data;
        }
    }
     ...
     *
     * @param array  $entity Searchanise data
     * @param array  $meta field names
     * @param object $data product data
     *
     * @return void
     */
    public function generateProductMetaFields(&$entry, $product_meta_fields_name, $data)
    {
     ...
     */
    public function getOptions($terms, $lang_code)
    {
        $options = array();

        foreach ($terms as $term) {
            if (ApiSe::getInstance()->isResultWidgetEnabled($lang_code)) {
                $options[] = html_entity_decode($term->name);
            } else {
                $options[] = $term->slug;
            }
        };
     ...
     * @return array
     */
    public function checkAttributtes($custom_attributes)
    {
        $attributtes = $this->generateCustomProductAttribute();
        $custom = [];
     ...
     * @return array
     */
    public function getTranslate($content, $lang_code)
    {
     ...
     * @return int
     */
    public function getQuantity($stock_status)
    {
        switch ($stock_status) {
            case 'instock':
                $quantity = 1;
     ...
     * @return array
     */
    public function getMetaProductTypes()
    {
        return $this->wpdb->get_results("SELECT post_title AS label, post_excerpt AS name FROM {$this->wpdb->prefix}posts WHERE post_excerpt IN(SELECT DISTINCT pm.meta_key FROM {$this->wpdb->prefix}postmeta pm LEFT JOIN {$this->wpdb->prefix}posts AS p ON pm.post_id = p.ID WHERE p.post_type = 'product' AND pm.meta_key NOT LIKE '\_%')");
    }
}
