	...
	public function woocommerce_checkout_update_order_review( $data ) {
		...
		if ( $allow_multi ) {
			...
			if ( ! $change ) {
				...
				if ( $currency_by_payment && $payment_method === 'ppcp-gateway' && $current_currency !== $currency_by_payment ) {
		...
		if ( $this->rate && $this->old_currency ) {
			foreach ( $tax_totals as &$value ) {
				if ( isset( $value->amount, $value->formatted_amount ) && $value->amount ) {
					...
					$value->formatted_amount = sprintf( "<div class='wmc-custom-checkout-left'>%s</div><div class='wmc-custom-checkout-right'>(%s)</div>", $prev_value, $value->formatted_amount );
				}
			}
		}
