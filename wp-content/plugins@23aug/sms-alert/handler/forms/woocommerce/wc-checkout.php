...
class WooCommerceCheckOutForm extends FormInterface {
	...
	public function handleForm() {
		...
		$this->popup_enabled             = ( (smsalert_get_option( 'checkout_show_otp_button', 'smsalert_general' ) !== 'on') || (smsalert_get_option( 'checkout_otp_popup', 'smsalert_general', 'off' ) === 'on') ) ? true : false;
		...
		add_action( 'woocommerce_order_partially_refunded', array( $this, 'refunded_partially_amount' ), 99, 2 );													 
		$this->routeData();
	}
	 /**
      ...
      * @return void
      */
    public function refunded_partially_amount($order_id, $refundedid){
		...
		$buyerMessage         =  WooCommerceCheckOutForm::pharse_sms_body($sms_data, $order_id);
		...
		if ( smsalert_get_option( 'admin_notification_' . $new_status, 'smsalert_general', 'on' ) === 'on' && ! empty( $admin_phone_number ) ) {
			...
			$adminMessage  =  WooCommerceCheckOutForm::pharse_sms_body($admin_sms_data, $order_id);
	...
	$checkout_otp_enabled = smsalert_get_option( 'buyer_checkout_otp', 'smsalert_general' );
     echo '
	<script>
	var signup_checkout = "'.get_option( 'woocommerce_enable_signup_and_login_from_checkout' ).'";
	...
	function smsalert() {
      ...
      {	jQuery("input[name=payment_method],input[name=radio-control-wc-payment-method-options]").each(function(e, t) {
		if(!post_verify)
		 {
            var o = jQuery(t).val();
	...
	function onChangePayment(payment)
	{
		if(!post_verify)
		{
		if((ask_otp && ((jQuery.inArray(payment, paymentMethods) > -1) || !selected_payment)) || ((1 == jQuery(".woocommerce #createaccount").prop("checked") || 0 == jQuery(".woocommerce #createaccount").length) && register_otp))
		{
			addShortcode();
		}
		else{
	...
	public function handle_failed_verification( $user_login, $user_email, $phone_number ) {
		...
		if ( isset( $_SESSION[ $this->form_session_var2 ] ) ) {
			...
			if ( (smsalert_get_option( 'checkout_show_otp_button', 'smsalert_general' ) !== 'on') || (smsalert_get_option( 'checkout_otp_popup', 'smsalert_general', 'off' ) === 'on') ) {
				wp_send_json( SmsAlertUtility::_create_json_response( SmsAlertMessages::showMessage( 'INVALID_OTP' ), 'error' ) );
				exit();
			} else {
...
class SA_CodTOPrepaid
{
    ...
    public function __construct()
    {
		...
		{
			add_action('cod_to_prepaid_cart_notification_sendsms_hook', array( $this, 'send_sms' ), 10);
			...
			add_action('admin_notices', array($this, 'display_wp_cron_warnings'), 10); 
		}
    ...
    { 
        ...
        'templates'        => $this->get_sms_alert_cod__templates(),
        );
     ...
     * @return array
     */
    public function get_sms_alert_cod__templates()
    {
