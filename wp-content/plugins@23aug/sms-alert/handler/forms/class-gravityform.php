...
class GF_SMS_Alert extends GFFeedAddOn
{
		...
		if(is_plugin_active('gravityview/gravityview.php'))
		{
			...
			foreach($statuses as $ks => $vs){
				$cst_fields[] =  array(
							'label'   => $vs,
							'type'    => 'checkbox',
							'name'    => 'smsalert_gform_status_ '. strtolower( $vs ),
							'class'   => 'mt-position-right',
    ...
    {    
        ...
        if (! empty($admin_nos) && ! empty($admin_msg) ) {
			...
			$admin_msg = $gf_admin_sms->parse_sms_body(
                $entry_id,
                $admin_msg
            );
...
class GF_Smsalert_Form extends FormInterface
{
     ...
     * @return string
     */
    public function parse_sms_body( $entry_id = '', $content = null)
    {
        $search        = array();
        $replace       = array();
        $entry = GFAPI::get_entry($entry_id);		
