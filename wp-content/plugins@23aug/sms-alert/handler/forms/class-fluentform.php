...
class FluentForm extends FormInterface {
	...
	public function handleForm(){
		...
		add_action( 'fluentform_after_form_render', array( $this, 'add_smsalert_shortcode' ), 10, 1 );
		add_filter('fluentform_is_form_renderable', array($this, 'add_smsalert_conversational'), 10, 2);
	}
		
	public function add_smsalert_conversational($renderable, $form){
		...
		if ( 'on' === $form_enable && 'on' === $otp_enable && '' !== $phone_field ){
			...
			$short_codes_js = do_shortcode( '[sa_verify phone_selector="' . esc_attr( $phone_field ) . '" submit_selector= ".ff-btn-submit-left .ff-btn" ]' );			
			wp_add_inline_script( "sa-handle-footer", $short_codes_js);				
			$conversational_fluent_js1 = '		
				jQuery(document).ready(function(){
					add_smsalert_button(".ff-btn-submit-left .ff-btn","input[name='.$phone_field.']","'.$uniqueNo.'");
					jQuery(document).on("click", "#sa_verify_'.$uniqueNo.'",function(event){
					event.preventDefault();
					send_otp(this,".ff-btn-submit-left .ff-btn","input[name='.$phone_field.']","","");
					});			
	...
	public function add_smsalert_shortcode($form){
		  ...
		  {		  
			if ( 'container' === $field['element'] ) {
				foreach ( $field['columns'] as $ffield ) {
					$variables = array_merge( $variables, self::create_variables( $ffield['fields'] ) );
				}
			} else {
				$variables = array_merge( $variables,self::create_variables( array($field) ));
			}
		  }
		}
	 ...
	 * Set variables.
	 *
	 * @param array $field fluent form fields array.
	 * @return array
	 */
