...
class SA_Easyappointments extends FormInterface
{
    ...
    {
        add_action('ea_new_app', array($this, 'send_sms_on_new_appointment'), 10, 2); 
        add_action('ea_edit_app', array($this, 'send_sms_on_update_appointment'), 10, 1);
        add_action('booking_reminder_sendsms_hook', array($this, 'send_reminder_sms'), 10);
		add_filter('ea_checkout_script', array($this, 'get_form_field'), 20);
    }
     ...
     * @return void
     */
    public static function set_booking_reminder($appointmentId)
    {  
                ...
                if ($customerMessage !== '' && $frequencyTime !==  0) {
                    ...
                    foreach ($rowsToPhone as $key => $data) {
                        ...
                        $booking = $mydatas->get_appintment_by_id($data['booking_id']);
							...
							$obj[$key]['sms_body']  = self::parse_sms_body($appointmentId, $booking, $customerMessage);
    ...
    public static function get_reminder_templates()
    {
        ...
        foreach ($schedulerData['cron'] as $key => $data) {
            ...
            $templates[$key]['token']          = self:: get_easyappointmentsvariables();
    ...
    public static function get_customer_templates()
    {
        ...
        foreach ($bookingStatuses as $ks => $vs) 
        {
            ...
            $templates[$ks]['token']          = self:: get_easyappointmentsvariables();
        }
        return $templates;
    }
    ...
    */
     ...
     * @return array
     */
    public function send_sms_on_new_appointment($appointmentId, $data)
    {
        $this->send_sms_on($appointmentId, $data); 
    }    
     ...
     * @return array
     */
    public function send_sms_on_update_appointment($appointmentId)
    {
      ...
      * @return void
      */
    public function send_sms_on($appointmentId,$data)
    {
        ...
        $this->set_booking_reminder($appointmentId);
        ...
        if(!empty($buyerNumber)) {
            ...
            if (($customerNotify === 'on' && $customerMessage !== '')) {
                $buyerMessage = $this->parse_sms_body($appointmentId, $datas, $customerMessage);
            ...
            if (empty($adminPhoneNumber) === false) {
                ...
                if ($adminNotify === 'on' && $adminMessage !== '') {
                    $adminMessage   = $this->parse_sms_body($appointmentId, $datas, $adminMessage);
     ...
     * @return string
     */
    public function parse_sms_body($appointmentId, $datas, $content = null)
    {
     ...
     * @return array
     */
    public static function get_easyappointmentsvariables()
    {
        $variable['[id]']                     = 'Id';
