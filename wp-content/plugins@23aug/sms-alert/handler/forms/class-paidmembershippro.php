...
class SA_Paidmembershipspro extends FormInterface
{
    ...
    {
        add_action('pmpro_before_change_membership_level', array( $this, 'send_sms_membership_cancel' ), 10, 4);
        add_action('pmpro_after_change_membership_level', array( $this, 'send_sms_membership_active' ), 10, 3);
        add_filter('sa_get_user_phone_no', array( $this, 'sa_update_billing_phone' ), 10, 2);
        add_action('pmpro_after_checkout', array( $this, 'trigger_sms_order_status_success' ), 10, 2);
        add_action('pmpro_order_status_cancelled', array( $this, 'trigger_sms_status_cancelled' ), 10, 2);
        add_action('pmpro_order_status_error', array( $this, 'trigger_sms_status_error' ), 10, 2);
        add_action('pmpro_order_status_pending', array( $this, 'triggar_sms_status_pending' ), 10, 2);
        add_action('pmpro_order_status_refunded', array( $this, 'trigger_sms_status_refunded' ), 10, 2);
        add_action('pmpro_order_status_review', array( $this, 'trigger_sms_status_review' ), 10, 2);
        add_action('pmpro_order_status_token', array( $this, 'trigger_sms_status_token' ), 10, 2);
        add_action('pmpro_checkout_after_user_fields', array( $this, 'add_phone_field' ), 10);
        add_action('pmpro_checkout_before_submit_button', array( $this, 'pmpro_form_checkout_otp' ), 10);
        add_filter('login_form_top', array( $this,'pmpro_form_login_otp' ), 10);
        add_action('smsalert_followup_sms',  array($this, 'send_reminder_sms'));
    }
     ...
     * @return void
     * */
    public function pmpro_form_checkout_otp()
    {
     ...
     * @return void
     * */
    public function pmpro_form_login_otp()
    {
     ...
     * @return void
     */
    public static function set_membership_reminder($order)
    { 
    ...
    {    
        ...
        if ('on' === $customerNotify && $subscription_details) { 
            foreach ( $subscription_details as $subscription ) 
            {
                ...
                $order_id                  = !empty($order->id) ? $order->id : "";
                $buyerMob                  = $order->billing->phone;
                $customer_msg               = $this->parse_sms_body($order, $customer_sms);
    ...
    public static function get_reminder_templates()
    {
        ...
        foreach ($schedulerData['cron'] as $key => $data) {
            ...
            $templates[$key]['token']          = self:: get_paid_memberships_provariables();
            $count++;
        }
        return $templates;
    }
    ...
    public static function get_customer_templates()
    {
		...
		$ws = 0;
         foreach ($memberStatuses as $wws => $ms){
			...
			$templates[$ws]['token']          = self::get_paid_memberships_provariables();
			$ws++;
		}
     ...
     * @return void
     */
    public function send_sms_membership_active($level_id, $user_id, $cancel_level)
    {
        if(!empty($level_id)) {
            ...
            if (($customerNotify === 'on' && $customerMessage !== '')) {
                  $buyerMessage = $this->parse_sms_body($data,  $customerMessage);
     ...
     * @return void
     */
    public function send_sms_membership_cancel($level_id, $user_id, $old_levels, $cancel_level)
    {
      ...
      * @return void
      */
    public function trigger_sms_order_status_success($user_id, $order)
    {
        $this->set_membership_reminder($order);
        $this->send_sms_on($order);
    } 
     ...
     * @return void
     */
    public function trigger_sms_status_cancelled($order,$user_id)
    {
        return $this->send_sms_on($order);
    }
     ...
     * @return void
     */
    public function trigger_sms_status_error($order,$user_id)
    {
        $this->send_sms_on($order);
    }
     ...
     * @return void
     */
    public function triggar_sms_status_pending($order,$user_id)
    {
        $this->send_sms_on($order);
    }
     ...
     * @return void
     */
    public function trigger_sms_status_refunded($order,$user_id)
    {
        $this->send_sms_on($order);
    }
     ...
     * @return void
     */
    public function trigger_sms_status_review($order,$user_id)
    {
        $this->send_sms_on($order);
    }
     ...
     * @return void
     */
    public function trigger_sms_status_token($order,$user_id)
    {
        $this->send_sms_on($order);
    }
     ...
     * @return void
     */
    public function send_sms_on($order)
    {
        if(!empty($order->billing)) {
            ...
            $buyerNumber            = get_user_meta($user_id, 'pmpro_bphone', true);
            if (($customerNotify === 'on' && $customerMessage !== '')) {
                $buyerMessage       = $this->parse_sms_body($order, $customerMessage);
                do_action('sa_send_sms', $buyerNumber, $buyerMessage);
            }
            // Send msg to admin.
            ...
            if (empty($adminPhoneNumber) === false) {
                ...
                if ($adminNotify === 'on' && $adminMessage !== '') {
                    $adminMessage   = $this->parse_sms_body($order, $adminMessage);
     ...
     * @return string
     */
    public function parse_sms_body($order, $content = null)
    {  
     ...
     * @return array
     */
    public static function get_paid_memberships_provariables()
    {
