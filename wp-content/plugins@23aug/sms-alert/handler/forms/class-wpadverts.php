...
class SA_WPadverts extends FormInterface
{
    ...
    public function handleForm()
    {
        ...
        add_action('advert_tmp_to_publish', array($this, 'send_sms_on_user_advert_publish'), 1000, 1);
        add_action('advert-pending_to_publish', array($this, 'send_sms_on_user_advert_publish'), 1000, 1);
        add_action('pending_to_publish', array($this, 'send_sms_on_advert_pending_to_publish'), 10, 1); 
        add_action('advert_tmp_to_advert-pending', array($this, 'send_sms_on_user_draft_to_pending'), 10, 1);
        add_action('advert_tmp_to_pending', array($this, 'send_sms_on_user_draft_to_pending'), 10, 1); 
        add_action('adverts_payment_new_to_pending', array($this, 'send_sms_on_user_draft_to_pending'), 10, 1); 
        add_action('publish_to_expired', array($this, 'send_sms_on_advert_publish_to_expire'), 10, 1);
        add_action('expired_to_publish', array($this, 'send_sms_on_advert_expired_to_publish'), 10, 1);
        add_action('expired_to_pending', array($this, 'send_sms_on_advert_expired_to_pending'), 10, 1);
        add_action('pending_to_trash', array($this, 'send_sms_on_advert_pending_to_trash'), 10, 1);    
        add_action('adverts_payment_completed', array( $this, 'adverts_post_updated'), 100);
        add_action( "adext_contact_form_send", array( $this, "send_sms_author" ), 10, 2 );		
     ...
     * @return void
     */
    public static function set_sms_reminder($post)
    {  
    ...
    {    
        ...
        if ('on' === $customerNotify && $subscription_details) {
            foreach ( $subscription_details as $subscription ) 
            {
                ...
                $buyerMob                  = get_post_meta($id, 'adverts_phone', true);
                $adverd                      = get_post($id);
                $customer_msg               = $this->parse_sms_body($adverd, $customer_sms);
    ...
    public static function add_tabs($tabs = array())
    {
		...
		$authorParam = array(
            ...
            'templates'        => self::get_author_templates(),
        );
        $admin_param = array(
    ...
    public static function get_reminder_templates()
    {
		...
		foreach ($schedulerData['cron'] as $key => $data) {
            ...
            $templates[$key]['token']          = self:: get_wpadvertsvariables();
            $count++;
        }
        return $templates;
    }
    ...
    public static function get_customer_templates()
    {
		...
		foreach ($adverdStatuses as $ks => $vs){
            ...
            $templates[$ks]['token']          = self:: get_wpadvertsvariables();
       }
        return $templates;
    } /**
     ...
     * @return array
     */
    public static function get_author_templates()
    {
     ...
     * @return array
     */
    public function send_sms_on_user_advert_publish($post)
    { 
        $this->set_sms_reminder($post);
        $this->send_sms_on($post);
    }
     ...
     * @return array
     */
    public function send_sms_on_user_draft_to_pending($post)
    { 
        $this->send_sms_on($post);
    }
     ...
     * @return array
     */
	public function send_sms_author($post_id, $form){
        ...
        if(!empty($authorNumber)) {
            ...
            if (($authorNotify === 'on' && $authorMessage !== '')) {
                $buyerMessage = $this->parse_sms_body($post, $authorMessage);
     ...
     * @return array
     */
    public function send_sms_on_advert_pending_to_publish($post)
    {
        $this->send_sms_on($post);
    }
     ...
     * @return array
     */
    public function send_sms_on_advert_publish_to_expire($post)
    {
     ...
     * @return array
     */
    public function send_sms_on_advert_expired_to_publish($post)
    {
        $expired         = get_post_meta($post->ID, '_expiration_date', true);
        $expiration_date =  date("Y-m-d H:i:s", $expired);
        $post->expired   = $expiration_date;
     ...
     * @return array
     */
    public function send_sms_on_advert_expired_to_pending($post)
    {
        $expired         = get_post_meta($post->ID, '_expiration_date', true);
        $expiration_date =  date("Y-m-d H:i:s", $expired);
        $post->expired   = $expiration_date;
     ...
     * @return array
     */
    public function send_sms_on_advert_pending_to_trash($post)
    {
        $this->send_sms_on($post);
    } 
     ...
     * @return array
     */
    public function send_sms_on_admin_advert_publish($post)
    {
        $this->send_sms_on($post);
    } 
     ...
     * @return array
     */
    public function adverts_post_updated($post)
    {
      ...
      * @return void
      */
    public function send_sms_on($post)
    {
        $id          = $post->ID;
        $status      = $post->post_status;
        ...
        if(!empty($authorNumber)) {
            ...
            if (($customerNotify === 'on' && $customerMessage !== '')) {
                $buyerMessage = $this->parse_sms_body($post, $customerMessage);
                do_action('sa_send_sms', $authorNumber, $buyerMessage);
            }
        }
		...
		if (empty($adminPhoneNumber) === false) {
			...
			if ($adminNotify === 'on' && $adminMessage !== '') {
				$adminMessage   = $this->parse_sms_body($post, $adminMessage);
     ...
     * @return string
     */
    public function parse_sms_body($adverd, $content = null)
    {
        $id              = $adverd->ID;
     ...
     * @return array
     */
    public static function get_wpadvertsvariables()
    {
        $variable['[id]']              = 'Id';
