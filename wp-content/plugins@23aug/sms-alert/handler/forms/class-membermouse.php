...
class SAMemberMouses extends FormInterface
{
    ...
    public function handleForm()
    {
        ...
        add_action('mm_bundles_add',              array($this, 'send_sms_on_status_bundles_added_to_member'));
    ...
    public function set_renewal_reminder( $data, $status )
    {
        ...
        if ('on' === $customer_notify && $subscription_details) {
            foreach ( $subscription_details as $subscription ) {
			   ...
			   $data = array(
				...
				$customer_msg	= $this->parse_sms_body($data, $customer_msg);
    ...
    public static function add_tabs($tabs = array())
    {
        ...
        $reminderParam = array(
            ...
            'templates'        => self::get_renewal_templates(),
        );
     ...
     * @return array
     * */
    public static function get_renewal_templates()
    {
        ...
        foreach ( $scheduler_data['cron'] as $key => $data ) {
            ...
            $templates[ $key ]['token']          = 
            self::get_membermousevariables('member,remindersms');
    ...
    public static function get_customer_templates()
    {
        ...
        $currentVal         = smsalert_get_option('customer_mm_notify_member_added', 'smsalert_mm_general', 'on');
            ...
            $templates[0]['token']          = self::get_membermousevariables('member');
        ...
        $currentVal      = smsalert_get_option('customer_mm_notify_mm_status', 'smsalert_mm_general', 'on');
            ...
            $templates[1]['token']          = self::get_membermousevariables('member');
        ...
        $currentVal      = smsalert_get_option('customer_mm_notify_membership_level', 'smsalert_mm_general', 'on');
            ...
            $templates[2]['token']          = self::get_membermousevariables('member');
        ...
        $currentVal      = smsalert_get_option('customer_mm_notify_bundle_status', 'smsalert_mm_general', 'on');
            ...
            $templates[3]['token']          = self::get_membermousevariables('member,bundle');
        ...
        $currentVal      = smsalert_get_option('customer_mm_notify_bundle_status', 'smsalert_mm_general', 'on');
            ...
            $templates[4]['token']          = self::get_membermousevariables('member,bundle');
        ...
        $currentVal      = smsalert_get_option('customer_mm_notify_mm_payment', 'smsalert_mm_general', 'on');
            ...
            $templates[5]['token']          = self::get_membermousevariables('member,order');
        ...
        $currentVal      = smsalert_get_option('customer_mm_notify_mm_payment_refund', 'smsalert_mm_general', 'on');
            ...
            $templates[6]['token']          = self::get_membermousevariables('member,order');
    ...
    public static function get_admin_templates()
    {
      ...
      * @return void
      */
    public function send_sms_on_status_member_add($data)
    {
        $status = $data['status_name'];
        $this->send_sms_on($data, $status, 'member_added');
        $this->set_renewal_reminder($data, $status);         
    }

    public function send_sms_on_status_member_membership_change($data)
    {
        $status = $data['status_name'];
        $this->send_sms_on($data, $status, 'membership_level');
        $this->set_renewal_reminder($data, $status); 
    }
    ...
    {
        $status = $data['status_name'];
        $this->send_sms_on($data, $status, 'mm_status');
		$this->set_renewal_reminder($data, $status); 
    } 
     ...
     * @return void
     */
    public function send_sms_on($data, $status,$entity)
    {
        ...
        if (empty($adminPhoneNumber) === false) {
            ...
            if ($adminNotify === 'on' && $adminMessage !== '') {
                $adminMessage   = $this->parse_sms_body($data, $adminMessage);
     ...
     * @return array
     */
    public static function get_membermousevariables($entity)
    {
