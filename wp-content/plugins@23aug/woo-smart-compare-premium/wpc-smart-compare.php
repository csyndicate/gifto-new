<?php
/*
Plugin Name: WPC Smart Compare for WooCommerce (Premium)
...
! defined( 'WOOSC_VERSION' ) && define( 'WOOSC_VERSION', '4.4.3' );
	...
	function woosc_init() {
		...
		if ( ! class_exists( 'WPCleverWoosc' ) && class_exists( 'WC_Product' ) ) {
			class WPCleverWoosc {
				...
				function init() {
					self::$fields = apply_filters( 'woosc_fields', self::$fields );
                    ...
                    <div class="wpclever_settings_page wrap">
                        ...
                        <div class="wpclever_settings_page_nav">
                            <h2 class="nav-tab-wrapper">
                                ...
                                -->
                                <a href="<?php echo esc_url( WOOSC_SUPPORT ); ?>" class="nav-tab" target="_blank">
									<?php esc_html_e( 'Support', 'woo-smart-compare' ); ?>
                                </a>
				...
				function get_table( $ajax = true ) {
					...
					if ( is_array( $products ) && ( count( $products ) > 0 ) ) {
							...
							foreach ( $saved_fields as $saved_field ) {
								switch ( $saved_field ) {
									...
									case 'availability':
										...
										break;
									case 'attributes':
										$attributes = apply_filters( 'woosc_attributes', get_option( 'woosc_attributes' ) );

										if ( is_array( $attributes ) && ( count( $attributes ) > 0 ) ) {
											foreach ( $attributes as $attribute ) {
												$attribute_pa      = wc_attribute_taxonomy_name( $attribute );
												$product_attribute = $product_obj->get_attribute( $attribute_pa );

												if ( empty( $product_attribute ) && $parent_product ) {
													$product_attribute = $parent_product->get_attribute( $attribute_pa );
												}

												$products_data[ $product_id ][ $attribute_pa ] = apply_filters( 'woosc_product_' . $attribute_pa, $product_attribute, $product_obj );
											}
										}

										break;
									case 'custom_attributes':
										$custom_attributes = explode( ',', apply_filters( 'woosc_custom_attributes', get_option( 'woosc_custom_attributes' ) ) );

										if ( ! empty( $custom_attributes ) ) {
											foreach ( $custom_attributes as $custom_attribute ) {
												$custom_attribute_name = trim( $custom_attribute );
												$custom_attribute_ca   = 'ca_' . sanitize_title( $custom_attribute_name );

												if ( $parent_product ) {
													$custom_attribute_val = $parent_product->get_attribute( $custom_attribute_name );
												} else {
													$custom_attribute_val = $product_obj->get_attribute( $custom_attribute_name );
												}

												$products_data[ $product_id ][ $custom_attribute_ca ] = apply_filters( 'woosc_product_' . $custom_attribute_ca, implode( ', ', explode( ' | ', $custom_attribute_val ) ), $product_obj );
											}
										}

										break;
									case 'custom_fields':
										if ( ( $custom_fields = apply_filters( 'woosc_custom_fields', get_option( 'woosc_custom_fields' ) ) ) && ! empty( $custom_fields ) ) {
											...
											if ( ! empty( $custom_fields_arr ) ) {
												foreach ( $custom_fields_arr as $custom_field ) {
													if ( ! empty( $custom_field ) ) {
														...
														$custom_field_cf   = 'cf_' . sanitize_title( $custom_field_name );
														$custom_field_val  = get_post_meta( $product_id, $custom_field_name, true ) ?: get_post_meta( $product_id, sanitize_title( $custom_field_name ), true );

														if ( ( strpos( $custom_field_cf, 'cf_wpcf' ) !== false ) && is_array( $custom_field_val ) ) {
															$custom_field_wpcf = '<ul>';

															foreach ( $custom_field_val as $cv ) {
																$custom_field_wpcf .= '<li>' . $cv[0] . '</li>';
															}

															$custom_field_wpcf .= '</ul>';

															$custom_field_val = $custom_field_wpcf;
														}

														$products_data[ $product_id ][ $custom_field_cf ] = apply_filters( 'woosc_product_' . $custom_field_cf, $custom_field_val, $product_obj );
													}
												}
											}
										}
						...
						foreach ( $saved_fields as $saved_field ) {
							...
							switch ( $saved_field ) {
								case 'attributes':
									$saved_attributes = apply_filters( 'woosc_attributes', get_option( 'woosc_attributes' ) );

									if ( is_array( $saved_attributes ) && ( count( $saved_attributes ) > 0 ) ) {
										foreach ( $saved_attributes as $saved_attribute ) {
											if ( ! isset( self::$attributes[ $saved_attribute ] ) ) {
												continue;
											}

											$saved_attribute    = esc_attr( $saved_attribute );
											$saved_attribute_pa = 'pa_' . $saved_attribute;
											$row                .= '<tr class="tr-attributes tr-' . ( $tr % 2 ? 'odd' : 'even' ) . ' tr-' . esc_attr( $saved_attribute_pa ) . ' ' . ( ! in_array( $saved_field, $cookie_fields ) ? 'tr-hide' : '' ) . '"><td class="td-label">' . self::$attributes[ $saved_attribute ] . '</td>';

											foreach ( $products_data as $product_data ) {
												if ( $product_data['name'] !== '' ) {
													if ( isset( $product_data[ $saved_attribute_pa ] ) ) {
														$row .= '<td>' . $product_data[ $saved_attribute_pa ] . '</td>';
													} else {
														$row .= '<td></td>';
													}
												} else {
									...
									break;
								case 'custom_attributes':
									$saved_custom_attributes = explode( ',', apply_filters( 'woosc_custom_attributes', get_option( 'woosc_custom_attributes' ) ) );

									if ( ! empty( $saved_custom_attributes ) ) {
										foreach ( $saved_custom_attributes as $saved_custom_attribute ) {
											$saved_custom_attribute_name = trim( $saved_custom_attribute );
											$saved_custom_attribute_ca   = 'ca_' . sanitize_title( $saved_custom_attribute_name );
											$row                         .= '<tr class="tr-fields tr-' . ( $tr % 2 ? 'odd' : 'even' ) . ' tr-' . esc_attr( $saved_custom_attribute_ca ) . ' ' . ( ! in_array( $saved_field, $cookie_fields ) ? 'tr-hide' : '' ) . '"><td class="td-label">' . $saved_custom_attribute_name . '</td>';

											foreach ( $products_data as $product_data ) {
												if ( $product_data['name'] !== '' ) {
													if ( isset( $product_data[ $saved_custom_attribute_ca ] ) ) {
														$row .= '<td>' . $product_data[ $saved_custom_attribute_ca ] . '</td>';
													} else {
														$row .= '<td></td>';
													}
												} else {
									...
									break;
								case 'custom_fields':
									$saved_custom_fields = explode( ',', apply_filters( 'woosc_custom_fields', get_option( 'woosc_custom_fields' ) ) );

									if ( ! empty( $saved_custom_fields ) ) {
										foreach ( $saved_custom_fields as $saved_custom_field ) {
											$saved_custom_field_arr   = explode( '|', $saved_custom_field );
											$saved_custom_field_name  = isset( $saved_custom_field_arr[0] ) ? trim( $saved_custom_field_arr[0] ) : '';
											$saved_custom_field_label = isset( $saved_custom_field_arr[1] ) ? trim( $saved_custom_field_arr[1] ) : $saved_custom_field_name;
											$saved_custom_field_cf    = 'cf_' . sanitize_title( $saved_custom_field_name );
											$row                      .= '<tr class="tr-fields tr-' . ( $tr % 2 ? 'odd' : 'even' ) . ' tr-' . esc_attr( $saved_custom_field_cf ) . ' ' . ( ! in_array( $saved_field, $cookie_fields ) ? 'tr-hide' : '' ) . '"><td class="td-label">' . $saved_custom_field_label . '</td>';

											foreach ( $products_data as $product_data ) {
												if ( $product_data['name'] !== '' ) {
													if ( isset( $product_data[ $saved_custom_field_cf ] ) ) {
														$row .= '<td>' . $product_data[ $saved_custom_field_cf ] . '</td>';
													} else {
														$row .= '<td></td>';
													}
												} else {
