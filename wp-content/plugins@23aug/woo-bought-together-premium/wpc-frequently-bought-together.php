...
! defined( 'WOOBT_VERSION' ) && define( 'WOOBT_VERSION', '4.3.1' );
		...
		if ( ! class_exists( 'WPCleverWoobt' ) && class_exists( 'WC_Product' ) ) {
			class WPCleverWoobt {
				...
				function __construct() {
					...
					if ( ( get_option( '_woobt_atc_button', 'main' ) === 'main' ) && ( $position !== 'none' ) ) {
					...
					add_action( 'woocommerce_add_to_cart', [ $this, 'add_to_cart' ], 10, 3 );
                                            ...
                                            <td>
                                                <select name="_woobt_atc_button">
                                                    ...
                                                    <option value="separate" <?php echo esc_attr( get_option( '_woobt_atc_button', 'main' ) === 'separate' ? 'selected' : '' ); ?>>
														<?php esc_html_e( 'Separate buttons', 'woo-bought-together' ); ?>
                                                    </option>
                                                </select>
                                            </td>
                                        </tr>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <select name="_woobt_plus_minus">
                                                    <option value="yes" <?php echo esc_attr( get_option( '_woobt_plus_minus', 'no' ) === 'yes' ? 'selected' : '' ); ?>>
														<?php esc_html_e( 'Yes', 'woo-bought-together' ); ?>
                                                    </option>
                                                    <option value="no" <?php echo esc_attr( get_option( '_woobt_plus_minus', 'no' ) === 'no' ? 'selected' : '' ); ?>>
														<?php esc_html_e( 'No', 'woo-bought-together' ); ?>
                                                    </option>
                                                </select>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <select name="_woobt_variations_selector">
                                                    ...
                                                    <option value="woovr" <?php echo esc_attr( get_option( '_woobt_variations_selector', 'default' ) === 'wpc_radio' || get_option( '_woobt_variations_selector', 'default' ) === 'woovr' ? 'selected' : '' ); ?>>
				...
				function enqueue_scripts() {
					...
					wp_localize_script( 'woobt-frontend', 'woobt_vars', array(
							...
							'this_item'                => get_option( '_woobt_show_this_item', 'yes' ),
							'counter'                  => get_option( '_woobt_counter', 'individual' ),
							'variation_selector'       => ( class_exists( 'WPClever_Woovr' ) && ( get_option( '_woobt_variations_selector', 'default' ) === 'wpc_radio' || get_option( '_woobt_variations_selector', 'default' ) === 'woovr' ) ) ? 'woovr' : 'default',
							...
							'add_to_cart'              => get_option( '_woobt_atc_button', 'main' ) === 'main' ? self::localization( 'add_to_cart', esc_html__( 'Add to cart', 'woo-bought-together' ) ) : self::localization( 'add_all_to_cart', esc_html__( 'Add all to cart', 'woo-bought-together' ) ),
				...
				function add_to_cart_validation( $passed, $product_id ) {
					if ( ( get_post_meta( $product_id, 'woobt_separately', true ) !== 'on' ) && self::get_ids( $product_id ) ) {
						if ( isset( $_REQUEST['woobt_ids'] ) || isset( $_REQUEST['data']['woobt_ids'] ) ) {
							...
							if ( ! empty( $items ) ) {
								foreach ( $items as $item ) {
									...
									if ( $item_product->is_sold_individually() && self::check_in_cart( $item['id'] ) ) {
				...
				function add_cart_item_data( $cart_item_data, $product_id ) {
					if ( ( isset( $_REQUEST['woobt_ids'] ) || isset( $_REQUEST['data']['woobt_ids'] ) ) && ( get_post_meta( $product_id, 'woobt_separately', true ) !== 'on' ) && ( self::get_ids( $product_id ) || ( get_option( '_woobt_default', 'none' ) !== 'none' ) ) ) {
				...
				function add_to_cart( $cart_item_key, $product_id, $quantity ) {
					if ( ( isset( $_REQUEST['woobt_ids'] ) || isset( $_REQUEST['data']['woobt_ids'] ) ) && ( self::get_ids( $product_id ) || ( get_option( '_woobt_default', 'none' ) !== 'none' ) ) ) {
						$ids = '';
				...
				function update_search_settings() {
					...
					update_option( '_woobt_search_types', array_map( 'sanitize_text_field', (array) $_POST['types'] ) );
				...
				function shortcode( $attrs ) {
					...
					ob_start();
					self::show_items( $attrs['id'], $attrs['custom_position'] );
					...
					if ( ! empty( $items ) ) {
						...
						if ( $is_separate_layout ) {
                            ...
                            <div class="woobt-images">
								<?php
								echo ' <div class="woobt-image woobt-image-this woobt-image-order-0 woobt-image-' . esc_attr( $product_id ) . '">' . $product->get_image() . '</div>';
                        ...
                        <div class="woobt-product woobt-product-this" data-order="0" data-qty="1" data-o_qty="1"
							...
							<?php if ( get_option( '_woobt_show_thumb', 'yes' ) !== 'no' ) { ?>
                                <div class="woobt-thumb">
                                    <div class="woobt-thumb-ori"><?php echo $product->get_image(); ?></div>
                            ...
                            <div class="woobt-title">
								...
								if ( ( $is_separate_atc || $is_custom_position ) && $product->is_type( 'variable' ) ) {
									if ( ( get_option( '_woobt_variations_selector', 'default' ) === 'wpc_radio' || get_option( '_woobt_variations_selector', 'default' ) === 'woovr' ) && class_exists( 'WPClever_Woovr' ) ) {
