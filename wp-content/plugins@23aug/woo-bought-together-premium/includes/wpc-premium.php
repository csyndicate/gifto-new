...
! defined( 'WOOBT_KEY' ) && define( 'WOOBT_KEY', 'g6k1O4m1' );

if ( ! function_exists( 'woobt_update_checker' ) ) {
	add_action( 'init', 'woobt_update_checker', 99 );
	function woobt_update_checker() {
		if ( defined( 'WOOBT_KEY' ) && wpc_allow_update( WOOBT_KEY ) ) {
			PucFactory::buildUpdateChecker( 'https://api.wpclever.net/update/' . WOOBT_KEY . '.json', WOOBT_FILE, 'woo-bought-together-premium' );
		}
	}
}
...
if ( ! class_exists( 'WPCleverPremium' ) ) {
	class WPCleverPremium {
		...
		function admin_enqueue_scripts() {
			wp_enqueue_style( 'wpc-premium', WOOBT_URI . 'assets/css/premium.css' );
			wp_enqueue_script( 'wpc-premium', WOOBT_URI . 'assets/js/premium.js', array( 'jquery', ), null, true );
		}
