...
class EPKB_Upgrades {
    ...
    public static function update_plugin_version() {
	    ...
	    EPKB_Utilities::save_wp_option( 'epkb_show_upgrade_message', true );

        // upgrade the plugin
        self::invoke_upgrades( $last_version );

        // update the plugin version
        $result = EPKB_Utilities::save_wp_option( 'epkb_version', Echo_Knowledge_Base::$version );
	...
	private static function upgrade_to_v612( &$kb_config ) {
		...
		EPKB_Utilities::save_kb_option( $kb_config['id'], EPKB_Icons::CATEGORIES_ICONS, $new_icons );
