...
class EPKB_AI_Help_Sidebar_Ctrl {
	...
	private function improve_readability() {
		...
		// let user see our error message if error received from OpenAI API response
		if ( ! empty( $openai_handler->error_message ) ) {
			self::ajax_show_error_die( __( 'An error occurred.', 'echo-knowledge-base' ) );
		}
	...
	private function save_settings() {
        ...
        if ( empty( $openai_api_key ) ) {
            self::ajax_show_error_die( __( 'Invalid api key', 'echo-knowledge-base' ) );
        }
	...
	private static function ajax_show_error_die( $message, $title = '', $error_code = '' ) {
		if ( defined('DOING_AJAX') ) {
			wp_die( json_encode( array( 'error' => true, 'message' => self::notification_box_bottom( $message, $title, 'error' ), 'error_code' => $error_code ) ) );
		}
	}
	...
	private function dismiss_main_intro() {
		...
		EPKB_Core_Utilities::update_kb_flag( 'ai_dismiss_main_intro', true );
