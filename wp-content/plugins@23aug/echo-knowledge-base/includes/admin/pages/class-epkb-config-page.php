	...
	private function get_kb_urls_config( $wizard_kb_config ) {
		...
		$kb_url_boxes[] = array(
			...
			'html' => EPKB_HTML_Forms::notification_box_middle( array(
					...
					'desc'  => sprintf( '%s <a href="%s" class="ret epkbfa epkbfa-external-link" target="_blank">%s</a>',
								esc_html__( 'Need to change KB URLs?', 'echo-knowledge-base' ), 'https://www.echoknowledgebase.com/documentation/changing-permalinks-urls-and-slugs/', esc_html__( 'Learn More', 'echo-knowledge-base' ) )
				), true  ),
		);
