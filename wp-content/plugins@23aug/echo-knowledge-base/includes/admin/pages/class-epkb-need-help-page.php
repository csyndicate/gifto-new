...
class EPKB_Need_Help_Page {
	...
	private function get_regular_views_config() {
		...
		if ( isset( $_GET['epkb_after_kb_setup'] ) && ! EPKB_Core_Utilities::is_kb_flag( 'epkb_after_kb_setup_notice_shown' ) ) {
			...
			EPKB_Core_Utilities::update_kb_flag( 'epkb_after_kb_setup_notice_shown' );
		}
	...
	private function getting_started_tab() {
		...
		if ( ! EPKB_Core_Utilities::is_run_setup_wizard_first_time() ) {
			...
			$steps_list[] = array(
				'icon_class' => '',
				'content_icon_class' => EPKB_Core_Utilities::is_kb_flag( 'settings_tab_visited' ) ? 'epkbfa epkbfa-check-circle' : '',
	...
	private static function steps_list_2_html( $kb_id ) {
		...
		<div class="epkb-admin__step-cta-box__column"><?php
			...
			if ( current_user_can( 'manage_categories' ) ) {
				...
				<a class="epkb-kb__wizard-link" href="<?php echo esc_url( $url ); ?>" target="_blank"><?php esc_html_e( 'Edit KB Categories', 'echo-knowledge-base' ); ?></a><?php
			} ?>
		</div>
		<div class="epkb-admin__step-cta-box__column"><?php
			...
			<a class="epkb-kb__wizard-link" href="<?php echo esc_url( $import_url ); ?>" target="_blank"><?php esc_html_e( 'Import CSV as Articles', 'echo-knowledge-base' ); ?></a>
			<a class="epkb-kb__wizard-link" href="<?php echo esc_url( $convert_url ); ?>" target="_blank"><?php esc_html_e( 'Convert Posts to Articles', 'echo-knowledge-base' ); ?></a>
		</div><?php
		return ob_get_clean();
	}
}