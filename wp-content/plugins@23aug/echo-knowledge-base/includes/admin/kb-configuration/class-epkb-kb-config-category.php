...
class EPKB_KB_Config_Category {
	...
	public function display_draft_fields( $category ) {
		...
		<tr class="form-field epkb-term-options-wrap">
			...
			<td><label><input type="checkbox" name="epkb_category_is_draft" <?php checked( $is_draft ); ?>> <span><?php echo esc_html__( 'Do not list categories on the KB Main Page or sidebars. It\'s important to note that categories ' .
	                                                            ...
	                                                            'prepare content for publication.', 'echo-knowledge-base' ); echo ' <strong>' . esc_html__( 'It is not intended to control access to KB content.', 'echo-knowledge-base' ) .
		...
		<<?php echo $is_new_category ? 'div' : 'tr'; ?> class="form-field epkb-term-options-wrap" >
			...
			<<?php echo $is_new_category ? 'div' : 'td'; ?>>				<?php 
				...
				<div class="epkb-term-options-message epkb-term-options-message--red" <?php echo $hide_block ? '' : ' style="display:none;" '; ?>>
                    <i class="epkbfa epkbfa-info-circle" aria-hidden="true"></i>
                    <p><?php echo esc_html( $hide_reason ); ?></p>
                </div>
	...
	public function save_category_fields( $term_id, $tt_id ) {
		...
		$categories_data = self::get_category_data_option( $this->kb_id );
		$old_term_icon = empty( $categories_data[$term_id] ) ? [] : $categories_data[$term_id];
		...
		// mark category
		if ( array_diff( $categories_data[$term_id], $old_term_icon ) ) {
			update_term_meta( $term_id, EPKB_Icons::CATEGORY_ICON_USER_CHANGED_FLAG, 1 );
		}

		EPKB_Utilities::save_kb_option( $this->kb_id, EPKB_Icons::CATEGORIES_ICONS, $categories_data );
	}
	...
	private function get_on_off_icons_link( $main_page_layout ) {
		...
		if ( EPKB_Core_Utilities::is_frontend_editor_hidden( $kb_config ) ) {
			return admin_url( 'edit.php?post_type=' . EPKB_KB_Handler::get_post_type( $this->kb_id ) . '&page=epkb-kb-configuration&wizard-features&preopen_setting=' .
			           ( $main_page_layout == 'Grid' ? 'grid_category_icon_location' : 'section_head_category_icon_location' ) );
		}

		$editor_urls = EPKB_Editor_Utilities::get_editor_urls( $kb_config, '', '', '', true, ( $main_page_layout == 'Grid' ? 'grid_category_icon_location' : 'section_head_category_icon_location' )  );
	...
	public static function get_category_data_option( $kb_id ) {
		...
		foreach( $categories_data as $term_id => $categories_icon ) {
			$categories_data[$term_id] = array(
				...
				'is_draft' => empty($categories_icon['is_draft']) ? false : true
			);
	...
	/**
	 * Backward capability. TODO: remove after few releases
	 */
