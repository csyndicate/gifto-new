...
class EPKB_Faqs_Shortcode {
	...
	public function __construct() {
		add_shortcode( 'epkb-faqs', array( $this, 'output_shortcode' ) );
	}
	...
	public function output_shortcode( $attributes ) {
		...
		$include = empty( $attributes['category_ids'] ) ? [] : explode( ',', $attributes['category_ids'] );
		...
		if ( empty( $include ) ) {
			$include = array_keys( $allowed_categories_ids ); // for empty categories parameters show all
		}

		// get current post id to exclude it from articles
		global $post;
		...
		<div class="epkb-faqs-container<?php echo esc_html( $faq_container_class ) . ' ' . esc_html( $presetClass ); ?>"><?php
			...
			foreach( $include as $include_category_id ) {
