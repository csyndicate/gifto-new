...
<nav class="woocommerce-pagination" data-type="<?php echo esc_attr( $pagination_type ); ?>">
	<?php if ( in_array( $pagination_type, [ 'load-more', 'infinite' ], true ) ) : ?>
		<?php if ( $total > $current ): ?>
			<?php
			$load_more_url = Minimog_Woo::instance()->get_shop_active_filters_url();
			$load_more_url = add_query_arg( 'product-page', $current + 1, $load_more_url );

			//custom code
			if($_GET['filterc_country'] && $_GET['filterc_country']!=""){
				$load_more_url = add_query_arg( 'filterc_country', $_GET['filterc_country'], $load_more_url );
			}
			//custom code
			?>
