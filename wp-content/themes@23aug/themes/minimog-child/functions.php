...
function addDingProduct($prd_info_array){
	...
	if(is_wp_error($inserted_post_id)){
		...
		update_post_meta($inserted_post_id, '_sku', $prd_info_array['SkuCode'].'D');
...
function addRewardProduct($prd_info_array){
	...
	if(is_wp_error($inserted_post_id)){
		...
		update_post_meta($inserted_post_id, '_sku','827'. $prd_info_array['productId'].'P');
		update_post_meta($inserted_post_id, '_virtual', 'yes');
		update_post_meta($inserted_post_id, '_regular_price', '1');
		update_post_meta($inserted_post_id, '_price', '1');
...
function update_reward_prd( $product_id, $prd_info_array) { 
	...
	//when product added, update the meta_data
		...
		wp_set_object_terms($inserted_post_id, 'egifts', 'product_cat');
		update_post_meta($product_id, '_sku', '827'.$prd_info_array['productId'].'P');
		update_post_meta($product_id, '_regular_price', '1');
		update_post_meta($product_id, '_price', '1');
...
function addreloadlyProduct($prd_info_array){
	...
	if(is_wp_error($inserted_post_id)){
	    ...
	    update_post_meta($inserted_post_id, 'fixedRecipientDenominations', '1');
	     update_post_meta($inserted_post_id, '_price', '1');
		 update_post_meta($inserted_post_id, '_sku','347'. $prd_info_array['productId'].'R');
		 
		
	   update_field('fromproduct_type', 'reloadly' , $inserted_post_id);
		update_field('rwd_product_api_id', $prd_info_array['productId'], $inserted_post_id);
		
		update_field('rwd_product_api_image',$prd_info_array['logoUrls'][0], $inserted_post_id);
		...
		//uploading and adding the feature image to the post
		$image = media_sideload_image( 'imageUrl', $inserted_post_id, $provi_prd['name'], 'id' );
		set_post_thumbnail( $inserted_post_id, $image );
		//$image = media_sideload_image( 'https://43523f7b3b.nxcli.net/wp-content/uploads/2022/08/images.png', $inserted_post_id, $prd_info_array['name'], 'id' );
		//set_post_thumbnail( $inserted_post_id, $image );
		$ret_array = array(
...
function update_giftcard_prd( $product_id, $prd_info_array) { 
	...
	wp_set_object_terms( $product_id, 'gifto', 'product_cat' );
		...
		update_post_meta($product_id, '_virtual', 'yes');
		update_post_meta($product_id, '_regular_price', '1');
		update_post_meta($product_id, '_sku','347'. $prd_info_array['productId'].'R');
	...
	update_field('fromproduct_type', 'reloadly' , $inserted_post_id);
		update_field('rwd_product_api_id', $prd_info_array['productId'], $inserted_post_id);
		update_field('rwd_product_api_image',$prd_info_array['logoUrls'][0], $product_id);
		...
		if (!empty($images)) {
	 	...
	 	$image = media_sideload_image( 'imageUrl', $inserted_post_id, $prd_info_array['name'], 'id' );
		//set_post_thumbnail( $product_id, $image );
}
	   ...
	   //echo "1";
			if (has_term( 'egifts', 'product_cat', $product_id )){
			  ...
			  echo "<pre>";
					...
					echo "</pre>"; 
					curl_close($curl);
                    echo  $xoxocode = $response->data->placeOrder->data->vouchers[0]->voucherCode; 
					echo $xoxopin = $response->data->placeOrder->data->vouchers[0]->pin;
					echo $xoxotrans = $response->data->placeOrder->data->orderId;		
					echo $xoxocat = "Plumpro";
					echo  $xoxosts ="successful";
				...
				global $wpdb;
					...
					$sqlxoxo = $wpdb->insert("vouchercards", array(
	               "created_at" =>$cr_date,				
			...
			//$accnumbr=get_field('AccountNumber',$product_id) 
             ...
             //echo $dng_numail;die(1);					
				...
				curl_close($curl);
					...
					$transidding = $response['TransferRecord']['TransferId']['TransferRef'];
					$statusd = "successful";
					$provider = "Ding";
					...
					$sqld = $wpdb->insert("vouchercards", array(
	               "created_at" =>$c_date,				
		...
		<table align="center" border="0" cellpadding="0" cellspacing="0" role="presentation" style="margin:0px auto;width:640px;max-width: 100%;font-family: "Montserrat", sans-serif;">
			...
			<tbody>
				<tr>
					<td>
						<!-- header table start -->
                        <table class="header-email" style="width: 100%;padding:0px;background:#ffffff;border-spacing:0px;margin: 25px 0;border: 1px solid #dfdfdf;">
                            <tbody>
                                <tr>
                                    <td align="left" style="width: 100%;width: 100%;background: #8054b3;text-align: left;padding: 30px;">
                                        <h1 style="color: #fff;font-size: 28px;font-weight: 400;">your voucher code here</h1>
                                    </td>
                                </tr>
                                <tr>
...
$prodnme =json_decode($response, true)['product']['productName'];
$patsts = json_decode($response, true)['status'];
//echo $patsts;
$mailsts = "pending";
$mailing1 = json_decode($response, true)['recipientEmail'];
$tran_created_time = json_decode($response, true)['transactionCreatedTime'];

//echo $mailing1;
$ttlprc = json_decode($response, true)['product']['totalPrice'];
//echo $ttlprc;
$curncy =json_decode($response, true)['product']['currencyCode'];
//echo $curncy;

curl_close($curl);
global $wpdb;
 
     $sql12 = $wpdb->insert("transaction_status", array(
	 "tran_time" => $tran_created_time,
...
function tchknwdev_add_custom_fields()
{
	...
	<div class="custom-price-buttons"> 
		<p><p>Below prices will be converted to equivalent value in AED at checkout.
			<?php 
...
if($product_type == 'reloadly'){ 
	...
	<div class="custom-price-buttons"> 
		<p>Below prices will be converted to equivalent value in AED at checkout.
			<?php 
				//$prd_curr = get_field('rwd_product_currency', $product->id);
				//if($prd_curr!="AED"): ?>
					</p>
...
function tchknwdev_add_item_data($cart_item_data, $product_id, $variation_id)
{
    ...
    {
        $cart_item_data['tchknwdev_rwdnumv_num'] = sanitize_text_field($_REQUEST['tchknwdev_rwdnumv_num']);
    } 
	
	if(isset($_REQUEST['tchknwdev_rwdnumv_cntry']))
    {
        $cart_item_data['tchknwdev_rwdnumv_cntry'] = sanitize_text_field($_REQUEST['tchknwdev_rwdnumv_cntry']);
    } 
    ...
    // if( !WC()->session->__isset( "reload_checkout" )) {
        ...
        foreach ( $cart_object->cart_contents as $key => $value ) {
            if( isset( $value["tchknwdev_rwdprice"] ) ) {
                ...
                $converted_price1 = convertCurrency($reward_price, $user_currency_code, 'AED');
...
function tchknwdev_add_custom_order_line_item_meta($item, $cart_item_key, $values, $order)
{
    ...
    {
        $item->add_meta_data('_tchknwdev_rwdnumv_num',$values['tchknwdev_rwdnumv_num']);
    } 
	if(array_key_exists('tchknwdev_rwdnumv_cntry', $values))
    {
        $item->add_meta_data('_tchknwdev_rwdnumv_cntry',$values['tchknwdev_rwdnumv_cntry']);
    } 
...
add_menu_page( 'Voucher Cards', ' Voucher Cards', 'manage_options', ' Voucher Cards', 'voucher_code_plugin','dashicons-editor-ul',100);
...
function show_text(){
	...
	echo "<div class ='cust_txt'>"."Tip: Want to see more choices? Change to 'show all countries' from filters." . "</div>";
}
...
add_filter( 'woocommerce_checkout_fields' , 'misha_labels_placeholders', 9999 );
 ...
 function misha_labels_placeholders( $f ) {
	...
	$f[ 'billing' ][ 'billing_phone' ][ 'label' ] = "Sender's Phone";
    ...
    <script type="text/javascript">
        ( function( $ ) {
            $( document.body ).on( 'updated_checkout', function(data) {
				...
				var countryCode = country_code.replace('+', '');
				var flag_code = $('#country-listbox li[data-dial-code="'+countryCode+'"]').attr('data-country-code');
				
				$('.iti-flag').removeClass().addClass('iti-flag');
				$('.iti-flag').addClass(flag_code);
                ...
                $.post( ajax_url, ajax_data, function( response ) { 
                    ...
                    $('.selected-dial-code').text(response);
					var countryCode = response.replace('+', '');
				//alert(countryCode);
                    var flag_code = $('#country-listbox li[data-dial-code="'+countryCode+'"]').attr('data-country-code');
					$('.iti-flag').removeClass().addClass('iti-flag');
					$('.iti-flag').addClass(flag_code);
