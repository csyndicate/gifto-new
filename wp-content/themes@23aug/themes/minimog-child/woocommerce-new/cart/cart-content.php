					...
					<tr class="woocommerce-cart-form__cart-item <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
						<td class="col-product-info">
							<div class="product-wrap">
								...
								<img class="single-product-cstm-image" src="<?php the_field( 'rwd_product_api_image', $product_id ) ?>" alt="Placeholder">
							<?php } ?>
						...
						<td class="product-quantity" data-title="<?php esc_attr_e( 'Quantity', 'minimog' ); ?>">
							...
							if ( $_product->is_sold_individually() ) {
								$product_quantity = sprintf( '1 <input type="hidden" name="%s" value="1" />', $cart_item_key );
							} else {
								ob_start();
